    ...
    for start in range(0, n, batch_size):
        ...
        if debug_first and debug_payload is None:
            debug_payload = {
                ...
                "first_r_over_rescale_R": (r[:5] / rescale_R).tolist(),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Evaluate ASSB cycle5 PINN against soft labels with debug checks")
    ...
    (out_dir / "debug_model_and_data.json").write_text(json.dumps({"debug_first_batch": debug_payloads, "datasets": metrics["datasets"], "model_params": metrics["model_params"], "output_indices": metrics["output_indices"]}, indent=2, ensure_ascii=False), encoding="utf-8")
