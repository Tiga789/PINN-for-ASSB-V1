...
Purpose
-------
Draw four interactive Matplotlib 3D surface figures for ASSB cycle5 ID101:
    1) cs_a true surface
...
The script is read-only. It does not train, evaluate, or overwrite model files.
It only reads evaluation arrays / soft-label arrays and saves optional PNGs.

How to run on Windows, from project root
----------------------------------------
    D:\\Anaconda\\envs\\torchgpu\\python.exe .\\plot_cs_surface_cycle5.py

If your evaluation npz has a non-standard filename, use:
    D:\\Anaconda\\envs\\torchgpu\\python.exe .\\plot_cs_surface_cycle5.py --eval_npz .\\EvalFin_101_cycle5_v4_cbarAC_potentialBaseline\\<your_arrays>.npz

Notes
-----
- Matplotlib interactivity depends on the backend. On Windows, TkAgg is usually available.
- Use --backend QtAgg or --backend TkAgg if your environment prefers one of them.
...
- All Matplotlib text is set to Times New Roman where available.
- The default surface colormap is coolwarm, i.e. blue-white-red.
"""
...
DEFAULT_OUTPUT_SUBDIR = "surface_cs_cycle5_ID101_matplotlib"
DEFAULT_CYCLE_LABEL = "cycle5_v4"
DEFAULT_MODEL_LABEL = "ID101"
SCRIPT_VERSION = "2026-05-05-times-new-roman-coolwarm"
...
def save_metrics_json(path: Path, metrics: Dict[str, Dict[str, float]], sources: Dict[str, str], args: argparse.Namespace) -> None:
    payload: Dict[str, Any] = {
        ...
        "plot_style": {
            ...
            "true_cmap": getattr(args, "true_cmap", None) or getattr(args, "cmap", DEFAULT_CMAP),
            "pred_cmap": getattr(args, "pred_cmap", None) or getattr(args, "cmap", DEFAULT_CMAP),
        },
        "plot_downsampling": {
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Interactive Matplotlib 3D surfaces for ID101 cycle5 cs_a/cs_c true/pred.")
    ...
    parser.add_argument("--output_dir", type=Path, default=None, help="Directory for optional PNG and metrics JSON outputs.")
    ...
    parser.add_argument("--backend", default="auto", help="Matplotlib backend: auto, TkAgg, QtAgg, Agg, etc. Default: %(default)s")
    parser.add_argument("--layout", choices=["separate", "grid", "both"], default="separate", help="Figure layout. Separate is smoother for dragging. Default: %(default)s")
    ...
    parser.add_argument("--cmap", default=DEFAULT_CMAP, help="Matplotlib colormap used by both true and predicted surfaces unless overridden. Default: %(default)s")
    parser.add_argument("--true_cmap", default=None, help="Optional colormap for truth surfaces only. Default: same as --cmap")
    parser.add_argument("--pred_cmap", default=None, help="Optional colormap for prediction surfaces only. Default: same as --cmap")
    parser.add_argument("--font_family", default=DEFAULT_FONT_FAMILY, help="Font family used for all Matplotlib figure text. Default: %(default)s")
    parser.add_argument("--elev", type=float, default=25.0, help="Initial 3D elevation angle. Default: %(default)s")
    parser.add_argument("--azim", type=float, default=-135.0, help="Initial 3D azimuth angle. Default: %(default)s")
    parser.add_argument("--no_save", action="store_true", help="Do not save PNG/JSON outputs.")
    parser.add_argument("--no_show", action="store_true", help="Do not show interactive figures; useful for smoke test/headless run.")
    parser.add_argument("--no_colorbar", action="store_true", help="Hide colorbar to improve interactive speed slightly.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    # Resolve paths relative to the script/project root, not the current shell path.
    ...
    setup_matplotlib_backend(args.backend, args.no_show)
    font_family = resolve_font_family(args.font_family)
    setup_matplotlib_style(font_family)
    true_cmap = args.true_cmap or args.cmap
    pred_cmap = args.pred_cmap or args.cmap
    ...
    print(f"[INFO] true cmap      = {true_cmap}")
    print(f"[INFO] pred cmap      = {pred_cmap}")
    ...
    except RuntimeError as pred_exc:
    ...
    if not args.no_save:
        save_metrics_json(output_dir / "cs_surface_metrics_ID101_cycle5.json", metrics, sources, args)

    print("\n[PLOT] Downsampled plotted grids")
    print(f"  cs_a: time {tidx_a.size}/{t_a.size}, radius {ridx_a.size}/{r_a.size}, surface polygons ≈ {(tidx_a.size-1)*(ridx_a.size-1)}")
    print(f"  cs_c: time {tidx_c.size}/{t_c.size}, radius {ridx_c.size}/{r_c.size}, surface polygons ≈ {(tidx_c.size-1)*(ridx_c.size-1)}")

    figs = []
    if args.layout in {"separate", "both"}:
        save = None if args.no_save else output_dir / "cs_a_true_surface.png"
        figs.append(plot_one_surface(
            ...
            z_label="concentration cs_a", zlim=zlim_a, cmap=true_cmap, save_path=save,
            elev=args.elev, azim=args.azim, show_colorbar=not args.no_colorbar, font_family=font_family,
        ))
        save = None if args.no_save else output_dir / "cs_a_pred_surface.png"
        figs.append(plot_one_surface(
            t=t_a_plot, r=r_a_plot, z=z_a_pred, variable="cs_a", role="pred", metrics=metrics["cs_a"],
            model_label=args.model_label, cycle_label=args.cycle_label, t_label=t_label, r_label=r_a_label,
            z_label="concentration cs_a", zlim=zlim_a, cmap=pred_cmap, save_path=save,
            elev=args.elev, azim=args.azim, show_colorbar=not args.no_colorbar, font_family=font_family,
        ))
        save = None if args.no_save else output_dir / "cs_c_true_surface.png"
        figs.append(plot_one_surface(
            ...
            z_label="concentration cs_c", zlim=zlim_c, cmap=true_cmap, save_path=save,
            elev=args.elev, azim=args.azim, show_colorbar=not args.no_colorbar, font_family=font_family,
        ))
        save = None if args.no_save else output_dir / "cs_c_pred_surface.png"
        figs.append(plot_one_surface(
            t=t_c_plot, r=r_c_plot, z=z_c_pred, variable="cs_c", role="pred", metrics=metrics["cs_c"],
            model_label=args.model_label, cycle_label=args.cycle_label, t_label=t_label, r_label=r_c_label,
            z_label="concentration cs_c", zlim=zlim_c, cmap=pred_cmap, save_path=save,
            elev=args.elev, azim=args.azim, show_colorbar=not args.no_colorbar, font_family=font_family,
        ))

    if args.layout in {"grid", "both"}:
        grid_data = {
            "cs_a_true": (t_a_plot, r_a_plot, z_a_true, metrics["cs_a"], zlim_a, r_a_label),
            "cs_a_pred": (t_a_plot, r_a_plot, z_a_pred, metrics["cs_a"], zlim_a, r_a_label),
            "cs_c_true": (t_c_plot, r_c_plot, z_c_true, metrics["cs_c"], zlim_c, r_c_label),
            "cs_c_pred": (t_c_plot, r_c_plot, z_c_pred, metrics["cs_c"], zlim_c, r_c_label),
        }
        save = None if args.no_save else output_dir / "cs_a_cs_c_true_pred_grid.png"
        figs.append(plot_grid(
            data=grid_data, model_label=args.model_label, cycle_label=args.cycle_label,
            t_label=t_label, r_label_map={"cs_a": r_a_label, "cs_c": r_c_label}, z_label="concentration",
            true_cmap=true_cmap, pred_cmap=pred_cmap, save_path=save,
            elev=args.elev, azim=args.azim, font_family=font_family,
        ))

    if args.no_show:
        print("[INFO] --no_show enabled; figures were not displayed.")
        return 0

    import matplotlib.pyplot as plt
    print("\n[INFO] Matplotlib windows are opening. Use left mouse drag to rotate, scroll to zoom, right mouse drag to pan.")
    plt.show(block=True)
    return 0
