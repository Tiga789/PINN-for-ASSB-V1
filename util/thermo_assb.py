...
VOLTAGE_ALIGNMENT_OFFSET_V = _summary_float(("voltage_alignment_offset_V", "voltage_offset_V"), -0.11588681607942332)
...
def _read_two_col_csv(path: Path, value_candidates: tuple[str, ...]) -> tuple[np.ndarray, np.ndarray] | None:
    if not path.exists():
        return None
    try:
...
def _interp1_torch(x: torch.Tensor, xp_np: np.ndarray, fp_np: np.ndarray) -> torch.Tensor:
    ...
    y0 = fp[idx_lo]
    y1 = fp[idx_hi]
    w = (xq - x0) / torch.clamp(x1 - x0, min=np.float64(1e-12))
...
def ds_c_fun(cs_c, T, R, cscamax):
    ...
    envelope = np.float64(DS_C_MIN_FACTOR) + (
        np.float64(1.0) - np.float64(DS_C_MIN_FACTOR)
    ) * torch.exp(
        -((theta - np.float64(DS_C_PEAK_CENTER)) / np.float64(DS_C_PEAK_WIDTH))
        ** np.float64(2.0)
    )
...
def phis_c0_fun(i0_a, j_a, F, R, T, Uocp_a0, j_c, i0_c, Uocp_c0):
    ...
    return (
        float(j_c) * (float(R) * float(T) / _to_tensor(i0_c, phie0))
        + _to_tensor(Uocp_c0, phie0)
        + phie0
    )
...
def setParams(params, deg, bat, an, ca, ic):
    ...
    initial_terminal_shift = float(params["I_discharge"]) * float(params["R_ohm_eff"]) + float(params["voltage_alignment_offset_V"])
    ...
    params["theta_a_min"] = np.float64(0.0)
    params["theta_a_max"] = np.float64(1.0)
    params["theta_c_min"] = np.float64(0.0)
    params["theta_c_max"] = np.float64(1.0)
    ...
    # Make provenance visible in smoke tests.
