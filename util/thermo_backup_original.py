...
def ds_c_degradation_param_fun(cs_c, T, R, cscamax, degradation_param):
    ...
    deg = tf.reshape(degradation_param, tf.shape(ds_c_nodeg))
    deg = tf.maximum(deg, np.float64(1e-12))
    return ds_c_nodeg / deg
...
def ds_c_fun_simp_degradation_param(cs_c, T, R, cscamax, degradation_param):
    deg = tf.reshape(degradation_param, tf.shape(cs_c))
    deg = tf.maximum(deg, np.float64(1e-12))
    return (
        np.float64(3.5e-15)
