...
Compared with the original project version, this file adds:
1) current_profile support
2) time_profile support
3) variable dt support inside the integration loop
4) graceful fallback when prettyPlot.progressBar is unavailable
"""
...
def integration(
    ...
    if verbose:
        print_progress_bar(0, n_t - 1, prefix="Step=", suffix="Start", length=20)
    ...
    for i_t in range(1, n_t):
            ...
            if EXACT_GRAD_DS_CS:
                ...
                Ds_c_tmp1 = _to_numpy(
                    params["D_s_c"](
                        np.clip(
                            sol["cs_c"][i_t - 1, :]
                            + np.ones(sol["cs_c"].shape[1], dtype=np.float64) * GRAD_STEP,
                            a_min=0,
        ...
        if verbose:
            print_progress_bar(i_t, n_t - 1, prefix="Step=", suffix="Complete", length=20)
