...
# -----------------------------------------------------------------------------
# ASSB current / surface-flux / radial-rescale helpers
# -----------------------------------------------------------------------------
...
def _current_at_t(params: dict, t_like: torch.Tensor) -> torch.Tensor:
    """Return applied cell current I(t) in A. +I = charge, -I = discharge."""
...
def _terminal_voltage_shift(params: dict, t_like: torch.Tensor) -> torch.Tensor:
    """Dynamic terminal-voltage shift used by the ASSB v3 soft-label generator."""
    t_like = _to_tensor(t_like)
...
def _surface_flux_from_current(params: dict, t_like: torch.Tensor, electrode: str) -> torch.Tensor:
    """SPM surface flux from applied current.
...
def _use_per_electrode_radial_rescale(params: dict) -> bool:
    flag = params.get("use_per_electrode_rescale_R", params.get("use_per_electrode_radial_rescale", True))
...
def _radial_rescale(params: dict, electrode: str) -> float:
    """Return electrode-specific radial input scale."""
    if not _use_per_electrode_radial_rescale(params):
        return float(params.get("rescale_R", max(float(params["Rs_a"]), float(params["Rs_c"]))))
    electrode = electrode.lower()
    if electrode.startswith("a") or electrode.startswith("n"):
        return float(params.get("rescale_R_a", params.get("rescale_R_negative", params.get("Rs_a"))))
    return float(params.get("rescale_R_c", params.get("rescale_R_positive", params.get("Rs_c"))))


def _concentration_span(params: dict, electrode: str, default_start: float, default_max: float) -> float:
    electrode = electrode.lower()
    if electrode.startswith("a") or electrode.startswith("n"):
        lower = float(params.get("cs_a_min", params.get("csanmin", 0.0)))
        upper = float(params.get("cs_a_upper", params.get("csanmax", default_max)))
    else:
        lower = float(params.get("cs_c_min", params.get("cscamin", 0.0)))
        upper = float(params.get("cs_c_upper", params.get("cscamax", default_max)))
    return max(abs(default_start - lower), abs(upper - default_start), abs(upper - lower), 1.0e-12)
...
def loss_fn_dynamicAttention_tensor(interiorTerms, boundaryTerms, dataTerms, regularizationTerms, int_col_weights, bound_col_weights, data_col_weights, reg_col_weights, alpha):
    ...
    return (int_loss + bound_loss + data_loss + reg_loss, int_loss_unweighted + bound_loss_unweighted + data_loss_unweighted + reg_loss_unweighted, int_loss, bound_loss, data_loss, reg_loss)
...
def get_unweighted_loss(self, int_col_pts, int_col_params, bound_col_pts, bound_col_params, reg_col_pts, reg_col_params, x_trainList, x_params_trainList, y_trainList, n_batch=1, tmax=None):
    accumulatedLoss = 0.0
    ...
    for i_batch in range(n_batch):
        ...
        loss_value, _, _, _, _ = loss_fn_lbfgs(interiorTerms_rescaled, boundaryTerms_rescaled, dataTerms_rescaled, regularizationTerms_rescaled, self.alpha_unweighted)
        accumulatedLoss += float(loss_value.detach().cpu())
    return accumulatedLoss / max(n_batch, 1)
...
def setResidualRescaling(self, weights):
    cs_a0 = float(self.params["cs_a0"])
    cs_c0 = float(self.params["cs_c0"])
    csanmax = float(self.params.get("csanmax", max(cs_a0, 1.0)))
    cscamax = float(self.params["cscamax"])
    Ds_a = float(self.params["D_s_a"](self.params["T"], self.params["R"]))
    Ds_c = float(self.params["D_s_c"](cs_c0, self.params["T"], self.params["R"], cscamax, np.float64(1.0)))
    j_a_abs = _reference_flux_abs(self.params, "a")
    j_c_abs = _reference_flux_abs(self.params, "c")
    C = max(abs(float(self.params.get("C", 1.0))), 1.0e-30)
    self.phie_transp_resc = 1.0 / j_a_abs
    self.phis_c_transp_resc = 1.0 / j_c_abs
    cs_a_span = _concentration_span(self.params, "a", cs_a0, csanmax)
    cs_c_span = _concentration_span(self.params, "c", cs_c0, cscamax)
    ...
    if self.activeInt:
        ...
        self.interiorTerms_rescale_unweighted = [abs(self.phie_transp_resc), abs(self.phis_c_transp_resc), abs(self.cs_a_transp_resc), abs(self.cs_c_transp_resc)]
        self.interiorTerms_rescale = [w_phie_int * self.interiorTerms_rescale_unweighted[0], w_phis_c_int * self.interiorTerms_rescale_unweighted[1], w_cs_a_int * self.interiorTerms_rescale_unweighted[2], w_cs_c_int * self.interiorTerms_rescale_unweighted[3]]
    else:
        self.interiorTerms_rescale_unweighted = [0.0]
        self.interiorTerms_rescale = [0.0]

    self.cs_a_bound_resc = Ds_a / j_a_abs
    self.cs_c_bound_resc = Ds_c / j_c_abs
    self.cs_a_bound_j_resc = Ds_a / j_a_abs
    self.cs_c_bound_j_resc = Ds_c / j_c_abs
    ...
    if self.activeBound:
        ...
        self.boundaryTerms_rescale_unweighted = [abs(self.cs_a_bound_resc), abs(self.cs_c_bound_resc), abs(self.cs_a_bound_j_resc), abs(self.cs_c_bound_j_resc)]
        self.boundaryTerms_rescale = [w_cs_a_rmin_bound * self.boundaryTerms_rescale_unweighted[0], w_cs_c_rmin_bound * self.boundaryTerms_rescale_unweighted[1], w_cs_a_rmax_bound * self.boundaryTerms_rescale_unweighted[2], w_cs_c_rmax_bound * self.boundaryTerms_rescale_unweighted[3]]
    else:
    ...
    Rs_a = float(self.params["Rs_a"])
    Rs_c = float(self.params["Rs_c"])
...
def data_loss(self, x_batch_trainList, x_cs_batch_trainList, x_params_batch_trainList, y_batch_trainList):
    ...
    resc_r_a = _radial_rescale(self.params, "a")
    resc_r_c = _radial_rescale(self.params, "c")
    ...
    out_phie = self.model([x_phie[:, self.ind_t : self.ind_t + 1] / resc_t, surfR_a / resc_r_a, self.rescale_param(p_phie[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], self.ind_deg_i0_a), self.rescale_param(p_phie[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1], self.ind_deg_ds_c)], training=True)
    phie_pred_rescaled = self.rescalePhie(out_phie[self.ind_phie], x_phie[:, self.ind_t : self.ind_t + 1], p_phie[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], p_phie[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1])
    ...
    surfR_c = float(self.params["Rs_c"]) * torch.ones_like(x_phis[:, self.ind_t : self.ind_t + 1])
    out_phis = self.model([x_phis[:, self.ind_t : self.ind_t + 1] / resc_t, surfR_c / resc_r_c, self.rescale_param(p_phis[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], self.ind_deg_i0_a), self.rescale_param(p_phis[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1], self.ind_deg_ds_c)], training=True)
    phis_c_pred_rescaled = self.rescalePhis_c(out_phis[self.ind_phis_c], x_phis[:, self.ind_t : self.ind_t + 1], p_phis[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], p_phis[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1])
    ...
    cs_a_pred_non_rescaled = self.model([x_csa[:, self.ind_t : self.ind_t + 1] / resc_t, x_csa[:, self.ind_r : self.ind_r + 1] / resc_r_a, self.rescale_param(p_csa[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], self.ind_deg_i0_a), self.rescale_param(p_csa[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1], self.ind_deg_ds_c)], training=True)[self.ind_cs_a]
    cs_a_pred_rescaled = self.rescaleCs_a(cs_a_pred_non_rescaled, x_csa[:, self.ind_t : self.ind_t + 1], x_csa[:, self.ind_r : self.ind_r + 1], p_csa[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], p_csa[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1], clip=False)
    ...
    cs_c_pred_non_rescaled = self.model([x_csc[:, self.ind_t : self.ind_t + 1] / resc_t, x_csc[:, self.ind_r : self.ind_r + 1] / resc_r_c, self.rescale_param(p_csc[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], self.ind_deg_i0_a), self.rescale_param(p_csc[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1], self.ind_deg_ds_c)], training=True)[self.ind_cs_c]
    cs_c_pred_rescaled = self.rescaleCs_c(cs_c_pred_non_rescaled, x_csc[:, self.ind_t : self.ind_t + 1], x_csc[:, self.ind_r : self.ind_r + 1], p_csc[:, self.ind_deg_i0_a : self.ind_deg_i0_a + 1], p_csc[:, self.ind_deg_ds_c : self.ind_deg_ds_c + 1], clip=False)
    return [[phie_pred_rescaled - y_phie], [phis_c_pred_rescaled - y_phis], [cs_a_pred_rescaled - y_csa], [cs_c_pred_rescaled - y_csc]]
...
def interior_loss(self, int_col_pts=None, int_col_params=None, tmax=None):
    ...
    deg_i0_scaled = self.rescale_param(deg_i0_a, self.ind_deg_i0_a)
    deg_ds_scaled = self.rescale_param(deg_ds_c, self.ind_deg_ds_c)
    output_a = self.model([t / resc_t, r_a / resc_r_a, deg_i0_scaled, deg_ds_scaled], training=True)
    output_c = self.model([t / resc_t, r_c / resc_r_c, deg_i0_scaled, deg_ds_scaled], training=True)
    output_surf_a = self.model([t / resc_t, rSurf_a / resc_r_a, deg_i0_scaled, deg_ds_scaled], training=True)
    output_surf_c = self.model([t / resc_t, rSurf_c / resc_r_c, deg_i0_scaled, deg_ds_scaled], training=True)
    ...
    eta_c = phis_c - phie - self.params["Uocp_c"](cse_c, self.params["cscamax"]) - _terminal_voltage_shift(self.params, t)
    if not self.linearizeJ:
    ...
    return [[j_a - j_a_rhs], [j_c - j_c_rhs], [cs_a_t - cs_a_r_r * ds_a - 2.0 * ds_a * cs_a_r / r_a - ds_a_r * cs_a_r], [cs_c_t - cs_c_r_r * ds_c - 2.0 * ds_c * cs_c_r / r_c - ds_c_r * cs_c_r]]
...
def boundary_loss(self, bound_col_pts=None, bound_col_params=None, tmax=None):
    ...
    resc_r_a = _radial_rescale(self.params, "a")
    resc_r_c = _radial_rescale(self.params, "c")
    deg_i0_scaled = self.rescale_param(deg_i0_a_bound, self.ind_deg_i0_a)
    deg_ds_scaled = self.rescale_param(deg_ds_c_bound, self.ind_deg_ds_c)
    r_0_a_bound = r_0_bound.clone().detach().requires_grad_(True)
    r_0_c_bound = r_0_bound.clone().detach().requires_grad_(True)
    r_max_a_bound = r_max_a_bound.clone().detach().requires_grad_(True)
    r_max_c_bound = r_max_c_bound.clone().detach().requires_grad_(True)
    output_r0_a_bound = self.model([t_bound / resc_t, r_0_a_bound / resc_r_a, deg_i0_scaled, deg_ds_scaled], training=True)
    output_r0_c_bound = self.model([t_bound / resc_t, r_0_c_bound / resc_r_c, deg_i0_scaled, deg_ds_scaled], training=True)
    output_rmax_a_bound = self.model([t_bound / resc_t, r_max_a_bound / resc_r_a, deg_i0_scaled, deg_ds_scaled], training=True)
    output_rmax_c_bound = self.model([t_bound / resc_t, r_max_c_bound / resc_r_c, deg_i0_scaled, deg_ds_scaled], training=True)
    cs_r0_a_bound = self.rescaleCs_a(output_r0_a_bound[self.ind_cs_a], t_bound, r_0_a_bound, deg_i0_a_bound, deg_ds_c_bound)
    cs_r0_c_bound = self.rescaleCs_c(output_r0_c_bound[self.ind_cs_c], t_bound, r_0_c_bound, deg_i0_a_bound, deg_ds_c_bound)
    ...
    cs_r0_a_bound_r = _grad(cs_r0_a_bound, r_0_a_bound)
    cs_r0_c_bound_r = _grad(cs_r0_c_bound, r_0_c_bound)
    ...
    return [[cs_r0_a_bound_r], [cs_r0_c_bound_r * deg_ds_c_bound], [time_gate * (cs_rmax_a_bound_r + j_a / ds_rmax_a_bound)], [time_gate * (cs_rmax_c_bound_r + j_c / ds_rmax_c_bound)]]


def _quadrature_s_points(params: dict, device, dtype) -> torch.Tensor:
    n_q = int(params.get("mass_reg_n_quad", params.get("N_MASS_REG_QUAD", 8)))
    n_q = max(n_q, 4)
    return torch.linspace(0.0, 1.0, n_q, dtype=dtype, device=device).reshape(1, n_q)


def _spherical_average_from_model(self, t: torch.Tensor, deg_i0_a: torch.Tensor, deg_ds_c: torch.Tensor, electrode: str):
    electrode = electrode.lower()
    s = _quadrature_s_points(self.params, self.device, t.dtype)
    n_q = s.shape[1]
    t_rep = t.repeat_interleave(n_q, dim=0)
    deg_i0_rep = deg_i0_a.repeat_interleave(n_q, dim=0)
    deg_ds_rep = deg_ds_c.repeat_interleave(n_q, dim=0)
    if electrode.startswith("a") or electrode.startswith("n"):
        Rs = float(self.params["Rs_a"])
        resc_r = _radial_rescale(self.params, "a")
        r = (s.repeat(t.shape[0], 1).reshape(-1, 1) * Rs).requires_grad_(True)
        out = self.model([t_rep / float(self.params["rescale_T"]), r / resc_r, self.rescale_param(deg_i0_rep, self.ind_deg_i0_a), self.rescale_param(deg_ds_rep, self.ind_deg_ds_c)], training=True)[self.ind_cs_a]
        cs = self.rescaleCs_a(out, t_rep, r, deg_i0_rep, deg_ds_rep)
    else:
        Rs = float(self.params["Rs_c"])
        resc_r = _radial_rescale(self.params, "c")
        r = (s.repeat(t.shape[0], 1).reshape(-1, 1) * Rs).requires_grad_(True)
        out = self.model([t_rep / float(self.params["rescale_T"]), r / resc_r, self.rescale_param(deg_i0_rep, self.ind_deg_i0_a), self.rescale_param(deg_ds_rep, self.ind_deg_ds_c)], training=True)[self.ind_cs_c]
        cs = self.rescaleCs_c(out, t_rep, r, deg_i0_rep, deg_ds_rep)
    cs_grid = cs.reshape(t.shape[0], n_q)
    weight = s.square().repeat(t.shape[0], 1)
    avg = 3.0 * torch.trapz(cs_grid * weight, s.repeat(t.shape[0], 1), dim=1).reshape(-1, 1)
    return avg


def regularization_loss(self, reg_col_pts=None, tmax=None):
    """Optional secondary conservation regularization for spherical particles.

    d<cs_j>/dt + 3 J_j(t) / R_j = 0. This is not data loss; it is a
    consequence of the spherical diffusion equation and the prescribed surface flux.
    It is active only when alpha[3] and BATCH_SIZE_REG are positive.
    """
    if not self.activeReg:
        return [[torch.tensor(0.0, dtype=torch.float64, device=self.device)]]
    tmin_reg = min(float(self.tmin_int_bound), float(self.tmax))
    if self.collocationMode == "random" or reg_col_pts is None:
        curr_tmax = float(tmax) if ((self.run_SGD and self.gradualTime_sgd) or (self.run_LBFGS and self.gradualTime_lbfgs)) and tmax is not None else float(self.tmax)
        t_reg = torch.empty((self.batch_size_reg, 1), dtype=torch.float64, device=self.device).uniform_(tmin_reg, curr_tmax)
    else:
        if (self.run_SGD and self.gradualTime_sgd) or (self.run_LBFGS and self.gradualTime_lbfgs):
            t_reg = self.stretchT(_to_tensor(reg_col_pts[self.ind_reg_col_t], device=self.device), tmin_reg, float(self.firstTime), tmin_reg, float(tmax))
        else:
            t_reg = _to_tensor(reg_col_pts[self.ind_reg_col_t], device=self.device)
    t_reg = t_reg.clone().detach().requires_grad_(True)
    deg_i0_a = torch.empty((t_reg.shape[0], 1), dtype=torch.float64, device=self.device).uniform_(float(self.params["deg_i0_a_min_eff"]), float(self.params["deg_i0_a_max_eff"]))
    deg_ds_c = torch.empty((t_reg.shape[0], 1), dtype=torch.float64, device=self.device).uniform_(float(self.params["deg_ds_c_min_eff"]), float(self.params["deg_ds_c_max_eff"]))
    csbar_a = _spherical_average_from_model(self, t_reg, deg_i0_a, deg_ds_c, "a")
    csbar_c = _spherical_average_from_model(self, t_reg, deg_i0_a, deg_ds_c, "c")
    dcsbar_a_dt = _grad(csbar_a, t_reg)
    dcsbar_c_dt = _grad(csbar_c, t_reg)
    j_a = _surface_flux_from_current(self.params, t_reg, "a")
    j_c = _surface_flux_from_current(self.params, t_reg, "c")
    Rs_a = float(self.params["Rs_a"])
    Rs_c = float(self.params["Rs_c"])
    time_gate = 1.0 - torch.exp(-t_reg / float(self.hard_IC_timescale))
    mass_a = time_gate * (dcsbar_a_dt + 3.0 * j_a / Rs_a)
    mass_c = time_gate * (dcsbar_c_dt + 3.0 * j_c / Rs_c)
    return [[mass_a], [mass_c]]
