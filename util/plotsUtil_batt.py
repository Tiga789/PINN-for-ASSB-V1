...
def line_cs_results(
    ...
    # Line colors (full color)
    n_stamp = len(time_stamps)
    cmap = cm.get_cmap("tab10")  # 也可以换 "viridis" / "turbo"
    if n_stamp <= 1:
        color_stamp = [cmap(0)]
    else:
        color_stamp = [cmap(i / (n_stamp - 1)) for i in range(n_stamp)]
...
def line_phi_results(
    ...
    fig = plt.figure()
    if plot_data:
        plt.plot(temp_dat, field_phie_dat, "-.", linewidth=1.6, color="b")
        plt.plot(temp_dat, field_phis_c_dat, "-.", linewidth=1.6, color="k")
...
def plotField_all(fieldList, fieldList2, xList, label, label2, name):
    ...
    for i in range(10):
        if i == 0:
            ...
            plt.plot(x_a, field_a[i * nt // 10, :], linewidth=1.6, color="k")
            if plotField2:
    ...
    for i in range(10):
        if i == 0:
            ...
            plt.plot(x_s, field_s[i * nt // 10, :], linewidth=1.6, color="k")
            if plotField2:
    ...
    for i in range(10):
        if i == 0:
            ...
            plt.plot(x_c, field_c[i * nt // 10, :], linewidth=1.6, color="k")
            if plotField2:
