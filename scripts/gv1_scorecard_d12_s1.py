#!/usr/bin/env python
"""Build D12-S1 6-profile scorecard from prediction.npz files.
...
- D12_S1_run_metrics.csv
- D12_S1_segment_metrics.csv
- D12_S1_mode_summary.csv
- D12_S1_candidate_decisions.csv
- D12_S1_scorecard_summary.json
"""
...
def main() -> None:
    ...
    ap.add_argument("--max_global_regress_V", type=float, default=0.010)
    ...
    out_dir = Path(args.output_dir) if args.output_dir else root / "D12_S1_scorecard"
    ...
    for mode in modes:
        ...
        promote = bool(low_ok and deep_ok and global_ok and corr_ok and rest_ok and high_ok)
        decisions.append(
            {
                "mode": mode,
    ...
    _write_csv(out_dir / "D12_S1_run_metrics.csv", run_rows)
    _write_csv(out_dir / "D12_S1_segment_metrics.csv", seg_rows)
    _write_csv(out_dir / "D12_S1_mode_summary.csv", mode_rows)
    _write_csv(out_dir / "D12_S1_candidate_decisions.csv", decisions)
    summary = {
        "ok": True,
        "runs_root": str(root),
    ...
    (out_dir / "D12_S1_scorecard_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
