...
def main() -> int:
    ap = argparse.ArgumentParser(description='Build D16-P5K-G train6/train8/train10 manifest from ALL55 P2Dlite-RG soft-label directory.')
    ...
    ap.add_argument('--train-set', default='', help='G_train12_rulev2_strict. Defaults to config default_train_set.')
    ...
    wanted = {(str(x['batch']), str(x['battery'])): {'reason': str(x.get('reason', f'{train_set}_selected')), 'split': str(x.get('split', 'core_train'))} for x in selected}
    rows = discover_profiles(soft_root)
    if len(rows) != 55:
        print(f'[D16-P5K-G manifest] WARNING: expected 55 profiles, found {len(rows)}', flush=True)

    found_train = set()
    for r in rows:
        key = (r['batch'], r['battery'])
        if key in wanted:
            r['split'] = wanted[key].get('split', 'core_train')
            r['reason'] = wanted[key].get('reason', f'{train_set}_selected')
            found_train.add(key)

    missing = sorted([{'batch': b, 'battery': bat, 'reason': meta['reason'], 'split': meta['split']} for (b, bat), meta in wanted.items() if (b, bat) not in found_train], key=lambda x: (x['batch'], x['battery']))
    train_count = sum(1 for r in rows if r['split'] in ('core_train', 'hard_probe', 'train'))
    hard_probe_count = sum(1 for r in rows if r['split'] == 'hard_probe')
    core_train_count = sum(1 for r in rows if r['split'] == 'core_train')
    ...
    report = {
        'stage': 'D16-P5K-G train12 profile-theta0 manifest',
        'softlabel_root': str(soft_root),
        'train_set': train_set,
        'profile_count': len(rows),
        'train_count': train_count,
        'core_train_count': core_train_count,
        'hard_probe_count': hard_probe_count,
        ...
        'notes': [
            ...
            'P5K-G train12 separates core_train and hard_probe; hard-probe cells are trained but reported separately, while the remaining cells stay held-out eval.'
        ]
    }
    with out_json.open('w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print('[D16-P5K-G manifest] status:', report['status'], 'train_set=', train_set, 'train_count=', train_count, 'eval_count=', eval_count, flush=True)
    print('[D16-P5K-G manifest] wrote:', out_csv, out_json, flush=True)
