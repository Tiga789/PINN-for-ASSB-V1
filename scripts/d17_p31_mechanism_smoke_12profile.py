...
from gv1.d17_pinn.p31_trainer import train_p31_mechanism_smoke


def main() -> None:
    ap = argparse.ArgumentParser(description="D17-P3.1 12-profile no-state-label mechanism smoke + voltage recovery audit")
    ap.add_argument("--config", default="configs/d17_pinn_rebuild_p31_12profile_smoke.json")
    ...
    cfg["d17_protocol_version"] = 31
    cfg["experiment_name"] = "d17_p31_12profile_mechanism_smoke"
    ...
    out_dir = args.out_dir or str(Path(str(cfg_get(cfg, "paths.output_root"))) / "p31_12profile_mechanism_smoke")
    summary = train_p31_mechanism_smoke(cfg, out_dir)
    print(json.dumps({
        ...
        "summary_json": str(Path(out_dir) / "D17_P31_12PROFILE_SMOKE_SUMMARY.json"),
        "loss_scale_audit_json": str(Path(out_dir) / "D17_P31_LOSS_SCALE_AUDIT.json"),
