...
$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1e_p2d_anchor_budget_23x200ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1e_p2d_anchor_budget_23x200ks_scorecard"
...
$Preflight = [ordered]@{
  stage = "D12-S1K 200ks source generation: quoted-args S1E-soft + baseline 23-profile 200ks"
...
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1K_sourcegen_23x200ks_preflight.json")
...
Write-Host "Next: run scripts\gv1_run_d12_s1k_apply_23profile_200ks.ps1 with -SourceRunsRoot `"$RunsRoot`""
