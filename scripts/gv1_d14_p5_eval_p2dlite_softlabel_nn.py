#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Evaluate D14-P5 XJTU P2Dlite soft-label NN smoke model."""
...
def main() -> int:
    ...
    if "train" not in by_split or "val" not in by_split or "test" not in by_split:
        fail_reasons.append("missing_train_val_or_test_metrics")
    ...
    report = {
        "package": "D14-P5 XJTU P2Dlite soft-label NN smoke evaluation",
    ...
    print(f"[P5 eval] status={status} profile_count={len(metrics_rows)} warn={warn_reasons} fail={fail_reasons}")
    return 0 if status == "PASS" or (status == "WARN" and args.allow_warn) else (2 if status == "WARN" else 1)
