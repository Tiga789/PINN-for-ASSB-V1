param(
  ...
  [int]$Seed = 42,
  [string]$Device = "auto",
  [string[]]$Modes = @("baseline_d951", "d12s1c_p2d_low_preserve_soft", "d12s1c_p2d_low_preserve_mid", "d12s1c_p2d_low_preserve_guarded"),
  [switch]$Clean,
  [switch]$Confirm200ks
)
...
if (-not $Confirm200ks) {
  throw "D12-S1C 200ks is gated. Run 6x40ks first and pass -Confirm200ks only if D12_S1C_candidate_decisions.csv promotes a candidate."
}
Set-Location $ProjectRoot

$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1c_p2d_balanced_6x200ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1c_p2d_balanced_6x200ks_scorecard"
...
function Get-ModeArgs([string]$Mode) {
  switch ($Mode) {
    ...
    "d12s1c_p2d_low_preserve_soft" {
      # Starts from S1 local_mild low-correction strength, but adds asymmetric
      # normal down-shift guards instead of S1B's over-strong suppression.
      return @(
        "--p2d_transport_scale_V", "0.120",
        "--p2d_max_correction_V", "0.40",
        "--p2d_protocol_gain", "0.15",
        "--p2d_transport_pred_center_V", "3.52",
        "--p2d_transport_pred_width_V", "0.21",
        "--p2d_low_gate_power", "1.00",
        "--p2d_pred_low_gate_power", "1.00",
        "--p2d_normal_suppression_center_V", "0.0",
        "--p2d_lowtarget_focus_weight", "0.22",
        "--p2d_deep_coverage_weight", "0.065",
        "--p2d_normal_preservation_weight", "0.62",
        "--p2d_normal_down_bias_guard_weight", "3.0",
        "--p2d_normal_down_shift_guard_weight", "4.0",
        "--p2d_normal_down_allowed_shift_V", "0.007",
    ...
    "d12s1c_p2d_low_preserve_mid" {
      return @(
        "--p2d_transport_scale_V", "0.140",
        "--p2d_max_correction_V", "0.45",
        "--p2d_protocol_gain", "0.18",
        "--p2d_transport_pred_center_V", "3.54",
        "--p2d_transport_pred_width_V", "0.22",
        "--p2d_low_gate_power", "1.00",
        "--p2d_pred_low_gate_power", "1.00",
        "--p2d_normal_suppression_center_V", "0.0",
        "--p2d_lowtarget_focus_weight", "0.26",
        "--p2d_deep_coverage_weight", "0.080",
        "--p2d_normal_preservation_weight", "0.76",
        "--p2d_normal_down_bias_guard_weight", "5.0",
        "--p2d_normal_down_shift_guard_weight", "7.0",
        "--p2d_normal_down_allowed_shift_V", "0.007",
        "--p2d_rest_preservation_weight", "0.23",
        "--p2d_high_preservation_weight", "0.20",
        "--p2d_correction_l2_weight", "0.058",
        "--p2d_preservation_huber_delta_V", "0.045"
      )
    }
    "d12s1c_p2d_low_preserve_guarded" {
      return @(
        "--p2d_transport_scale_V", "0.160",
        "--p2d_max_correction_V", "0.50",
        "--p2d_protocol_gain", "0.22",
        "--p2d_transport_pred_center_V", "3.56",
        "--p2d_transport_pred_width_V", "0.23",
        "--p2d_low_gate_power", "1.00",
        "--p2d_pred_low_gate_power", "1.00",
        "--p2d_normal_suppression_center_V", "0.0",
        "--p2d_lowtarget_focus_weight", "0.30",
        "--p2d_deep_coverage_weight", "0.095",
        "--p2d_normal_preservation_weight", "0.90",
        "--p2d_normal_down_bias_guard_weight", "8.0",
        "--p2d_normal_down_shift_guard_weight", "11.0",
        "--p2d_normal_down_allowed_shift_V", "0.007",
        "--p2d_rest_preservation_weight", "0.26",
        "--p2d_high_preservation_weight", "0.22",
        "--p2d_correction_l2_weight", "0.075",
        "--p2d_preservation_huber_delta_V", "0.045"
      )
    }
...
$Preflight = [ordered]@{
  stage = "D12-S1C balanced low-normal 6-profile 200ks preflight"
...
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1C_6profile_200ks_preflight.json")
...
& $PythonExe -m py_compile `
  ...
  (Join-Path $ProjectRoot "scripts\gv1_scorecard_d12_s1c.py")
...
& $PythonExe (Join-Path $ProjectRoot "scripts\gv1_scorecard_d12_s1c.py") `
  --runs_root $RunsRoot `
...
Write-Host "D12-S1C 6x200ks done. Scorecard: $ScorecardDir"
