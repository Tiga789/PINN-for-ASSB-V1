param(
  ...
  [int]$Seed = 42,
  [string]$Device = "auto",
  [string[]]$Modes = @("baseline_d951", "d12s1_p2d_protocol_guarded"),
  [switch]$Confirm200ks,
  [switch]$Clean
)
...
if (-not $Confirm200ks) {
  throw "D12-S1 200ks is gated. Run 6x40ks first and pass -Confirm200ks only if D12_S1_candidate_decisions.csv promotes a candidate."
}
Set-Location $ProjectRoot

$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1_train_inside_p2d_6x200ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1_train_inside_p2d_6x200ks_scorecard"
...
function Get-ModeArgs([string]$Mode) {
  switch ($Mode) {
    ...
    "d12s1_p2d_local_mild" {
      return @("--p2d_transport_scale_V", "0.12", "--p2d_protocol_gain", "0.15", "--p2d_lowtarget_focus_weight", "0.18", "--p2d_deep_coverage_weight", "0.05", "--p2d_normal_preservation_weight", "0.42", "--p2d_rest_preservation_weight", "0.18", "--p2d_high_preservation_weight", "0.15", "--p2d_correction_l2_weight", "0.030")
    }
    "d12s1_p2d_protocol_guarded" {
      return @("--p2d_transport_scale_V", "0.18", "--p2d_protocol_gain", "0.25", "--p2d_lowtarget_focus_weight", "0.25", "--p2d_deep_coverage_weight", "0.08", "--p2d_normal_preservation_weight", "0.55", "--p2d_rest_preservation_weight", "0.20", "--p2d_high_preservation_weight", "0.18", "--p2d_correction_l2_weight", "0.035")
    }
    "d12s1_p2d_lowtarget_focus" {
      return @("--p2d_transport_scale_V", "0.22", "--p2d_protocol_gain", "0.35", "--p2d_lowtarget_focus_weight", "0.30", "--p2d_deep_coverage_weight", "0.10", "--p2d_normal_preservation_weight", "0.65", "--p2d_rest_preservation_weight", "0.25", "--p2d_high_preservation_weight", "0.20", "--p2d_correction_l2_weight", "0.045")
    }
...
$Preflight = [ordered]@{
  stage = "D12-S1 6-profile 200ks preflight"
  ...
  Modes = $Modes
  Confirm200ks = [bool]$Confirm200ks
  Battery8Excluded = $true
  MetadataOn = $false
  HardClampDisabled = $true
}
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1_6x200ks_preflight.json")
...
foreach ($profile in $Profiles) {
  ...
  foreach ($mode in $Modes) {
    ...
    if ($mode -eq "baseline_d951") {
      $cmd = @((Join-Path $ProjectRoot "scripts\gv1_train_conditioned_pinn.py"), "--solution_npz", $npz, "--output_dir", $out, "--profile_adaptive_mode", "auto", "--epochs", "$Epochs", "--batch_size", "$BatchSize", "--max_time_points", "$MaxTimePoints", "--time_window_s", "200000", "--prediction_time_points", "$PredictionTimePoints", "--prediction_radial_points", "64", "--seed", "$Seed", "--device", "$Device", "--enable_voltage_hard_clamp", "false")
    } else {
      $cmd = @((Join-Path $ProjectRoot "scripts\gv1_train_d12_s1_p2d_local.py"), "--solution_npz", $npz, "--output_dir", $out, "--profile_adaptive_mode", "auto", "--epochs", "$Epochs", "--batch_size", "$BatchSize", "--max_time_points", "$MaxTimePoints", "--time_window_s", "200000", "--prediction_time_points", "$PredictionTimePoints", "--prediction_radial_points", "64", "--seed", "$Seed", "--device", "$Device", "--enable_voltage_hard_clamp", "false") + (Get-ModeArgs $mode)
    }
...
Write-Host "D12-S1 6x200ks done. Scorecard: $ScorecardDir"
