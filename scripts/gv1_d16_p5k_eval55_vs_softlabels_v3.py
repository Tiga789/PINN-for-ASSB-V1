...
def extract_npy_member(npz_path: Path, key: str, cache_root: Path) -> Path:
    ...
    try:
        ...
        raise FileNotFoundError(f'P5K eval cache/source file missing or path too long. npz={npz_path}; key={key}; cache_root={cache_root}; dst={dst}') from exc
    return dst
...
def feature_chunk(t: np.ndarray, I: np.ndarray, V: np.ndarray, s: int, e: int, stats: Dict[str, float], qn_full: np.ndarray) -> np.ndarray:
    ...
    X = np.stack([
        ...
        In, np.abs(In).astype(np.float32), dI, qn_full[s:e].astype(np.float32), vn, dV,
...
def hard_baseline_from_observed(x_raw: torch.Tensor, cfg: Dict[str, Any]) -> Dict[str, torch.Tensor]:
    ...
    base_a = torch.clamp(float(h.get('theta_a_mid', 0.405)) + float(h.get('theta_a_amplitude', 0.245))*centered, float(h.get('theta_a_min', 0.02)), float(h.get('theta_a_max', 0.96)))
    base_c = torch.clamp(float(h.get('theta_c_mid', 0.610)) - float(h.get('theta_c_amplitude', 0.245))*centered, float(h.get('theta_c_min', 0.02)), float(h.get('theta_c_max', 0.96)))
    return {'theta_a_base': base_a, 'theta_c_base': base_c, 'phase': phase}
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5K v3 evaluate hard-cbar/OCP residual model on all55 against P2Dlite-RG soft labels. Soft labels are evaluation-only.')
    ...
    print(f'[D16-P5K eval v3] mmap_cache_root={cache_root}', flush=True)
    ...
    for row in rows:
        ...
        try:
            ...
            for s in range(0, n, int(args.chunk_size)):
                ...
                if s == 0 or e == n:
                    print(f"[D16-P5K eval v3] {meta['profile_id']}: chunk {s}:{e}/{n}", flush=True)
            ...
            print(f"[D16-P5K eval v3] FAIL {meta.get('profile_id')}: {repr(exc)}", flush=True)
    ...
    write_csv(metrics_rows, out_dir / 'D16_P5K_METRICS_BY_PROFILE.csv')
    write_csv(aggregate_rows('split:'), out_dir / 'D16_P5K_SPLIT_METRICS.csv')
    write_csv(aggregate_rows('batch:'), out_dir / 'D16_P5K_BATCH_METRICS.csv')
    write_csv(aggregate_rows('protocol:'), out_dir / 'D16_P5K_PROTOCOL_METRICS.csv')
    write_json(failures, out_dir / 'D16_P5K_FAILURES.json')

    score = {
        'stage': 'D16-P5K hard-cbar/OCP residual evaluation',
        ...
        'notes': [
            ...
            'P5K uses hard cbar/OCP-style baseline plus bounded residual; it avoids P5G heuristic theta-gap targets.'
        ],
        'failures': failures,
    }
    write_json(score, out_dir / 'D16_P5K_FINAL_SCORECARD.json')
    print('[D16-P5K eval v3] operational_status:', score['operational_status'], 'evaluated=', len(metrics_rows), 'failures=', len(failures), flush=True)
    print('[D16-P5K eval v3] wrote:', out_dir / 'D16_P5K_FINAL_SCORECARD.json', flush=True)
