...
$outDir = Join-Path $Root ("ModelFin_112_v7_softscore_seed{0}_quicksmoke" -f $Seed)
$logDir = Join-Path $Root "LogFin_112_v7_quicksmoke"
New-Item -ItemType Directory -Force $logDir | Out-Null
$logPath = Join-Path $logDir ("seed_{0}.log" -f $Seed)

if ($Clean -and (Test-Path $outDir)) { Remove-Item $outDir -Recurse -Force }

Write-Host "ASSB-112 v7 quick smoke"
Write-Host "Purpose   = fast progress/code check, not final model"
Write-Host "Selection = softscore, hard guard audit-only"
Write-Host "Seed      = $Seed"
Write-Host "Epochs    = $Epochs"
Write-Host "PrintEvery= $PrintEvery"
...
& $Python (Join-Path $Root "scripts\train_assb111_soh_head.py") `
  ...
  --patience 800 `
  --min_epochs_before_patience $Epochs `
  ...
  --candidate_tag ("v7_quicksmoke_seed{0}" -f $Seed) `
  --protocol_tag "ASSB112_v7_softscore_quicksmoke_trainval_only" `
  2>&1 | Tee-Object -FilePath $logPath

if ($LASTEXITCODE -ne 0) {
  Write-Host "[FAILED] quick smoke seed=$Seed exit=$LASTEXITCODE"
  exit $LASTEXITCODE
}
Write-Host "[OK] quick smoke finished."
