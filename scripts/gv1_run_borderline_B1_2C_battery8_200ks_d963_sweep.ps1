...
$outRoot = "E:\XJTU battery dataset\_gv1_cache\xjtu_batch134_train_conditioned_pinn_borderline_B1_2C_battery8_200ks_d963_sweep"
...
$runs = @(
  ...
  @{name="D_lower_lr_seed2026_lr5e4";   seed=2026; lr=0.0005;  epochs=1600},
  @{name="E_slow_seed42_lr35e5";        seed=42;   lr=0.00035; epochs=1800},
  @{name="F_slow_seed7_lr35e5";         seed=7;    lr=0.00035; epochs=1800}
)
foreach ($r in $runs) {
  $out = Join-Path $outRoot $r.name
  Write-Host ""; Write-Host "===== D9.6.3 sweep $($r.name) ====="
...
$scorecard = Join-Path $outRoot "scorecard_borderline_200ks_d963_sweep.json"
