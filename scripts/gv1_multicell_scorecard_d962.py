#!/usr/bin/env python
"""Build a D9.6.2 multi-profile scorecard from gv1_prediction_metrics.py output."""
...
def _row_status(row: dict[str, Any], window: str) -> tuple[str, list[str]]:
    ...
    # D9.6.2 is a verification step. We keep thresholds realistic for cross-cell checks,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Create D9.6.2 multi-profile scorecard.")
    ...
    if fail_count > 0:
        overall = "fail"
        recommendation = "暂停扩窗；先检查 fail profile 的协议/低压段/高压饱和；不要直接扩窗。"
    elif borderline_count > 0:
        overall = "borderline_continue_carefully"
        recommendation = "可以少量扩窗，但不要直接 24-profile 200ks；优先检查 borderline/late-2C profile。"
    else:
        overall = "pass"
        recommendation = "D9.6.2 scorecard 通过；若当前是 single/borderline repair，再考虑 24-profile 40ks/小批量 200ks。"

    out = {
        "ok": bool(rows),
        "stage": "GV1 D9.6.2 multi-profile verification scorecard",
