...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5B FAST train6 observation-physics model. No internal soft-label data loss is used.')
    ...
    for r in train_rows:
        ...
        print(f"[D16-P5B fast train] loaded observed-only {r['profile_id']}: n={n} train={Xtr.shape[0]} val={Xva.shape[0]}", flush=True)
    ...
    with history_path.open('w', newline='', encoding='utf-8') as f:
        ...
        for ep in range(1, epochs + 1):
            ...
            if ep % max(1, val_every) == 0 or ep == 1:
                msg = f'[D16-P5B fast train] epoch={ep} train={train_loss:.6g}'
                if np.isfinite(val_loss):
                    msg += f' val={val_loss:.6g} best_epoch={best_epoch} best_val={best_val:.6g}'
                print(msg, flush=True)
            if bad_validations >= patience:
                print(f'[D16-P5B fast train] early stop at epoch={ep}; best_epoch={best_epoch} best_val={best_val:.6g}', flush=True)
                break

    summary = {
        'stage': 'D16-P5B FAST train6 observation-physics training',
        ...
        'notes': [
            ...
            'This fast trainer keeps sampled X_train/X_val tensors on GPU and validates every val_every epochs.'
        ]
    }
    ...
    print('[D16-P5B fast train] status:', summary['status'], 'checkpoint:', summary['checkpoint'], flush=True)
