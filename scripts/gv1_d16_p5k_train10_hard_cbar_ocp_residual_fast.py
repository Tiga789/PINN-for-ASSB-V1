...
# D16-P5K: hard cbar / OCP-style residual model.
...
def hard_baseline_from_observed(x_raw: torch.Tensor, cfg: Dict[str, Any]) -> Dict[str, torch.Tensor]:
    ...
    # observed OCP-like phase variable: high voltage and positive cumulative charge both imply larger anode stoichiometry and lower cathode stoichiometry.
    soc_v = torch.sigmoid(vg * v_z)
    soc_q = 0.5 + 0.5 * torch.tanh(qg * q_z)
    phase = torch.clamp(wv * soc_v + wq * soc_q, 0.0, 1.0)
...
def physics_observation_loss(raw: torch.Tensor, x_std: torch.Tensor, x_raw: torch.Tensor, cfg: Dict[str, Any], teacher_raw: torch.Tensor | None = None) -> Tuple[torch.Tensor, Dict[str, float]]:
    ...
    if teacher_raw is not None:
        ...
        # The older raw output can be mapped through the same P5K transform for branch compatibility.
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5K FAST train hard-cbar/OCP residual model. No internal-state soft-label data loss is used.')
    ...
    if len(train_rows) not in (6, 8, 10):
        raise ValueError(f'Expected 6/8/10 train rows, got {len(train_rows)}')
    ...
    for r in train_rows:
        ...
        print(f"[D16-P5K train] loaded observed-only {r['profile_id']}: n={n} train={Xtr.shape[0]} val={Xva.shape[0]}", flush=True)
    ...
    print('[D16-P5K train] warm_start:', warm_info, flush=True)
    ...
    for ep in range(1, epochs + 1):
        ...
        if ep == 1 or ep % val_every == 0 or ep == epochs:
            ...
            history.append(row)
            print(f"[D16-P5K train] epoch={ep} train={train_loss:.6g} val={val_loss:.6g} best={best_val:.6g}", flush=True)
            if val_loss < best_val:
                best_val = val_loss
                best_epoch = ep
                ckpt = {
                    'stage': 'D16-P5K hard-cbar OCP residual model',
    ...
    write_json({'stage': 'D16-P5K train input audit', 'rows': audit_rows}, out_dir / 'D16_P5K_TRAIN_INPUT_AUDIT.json')
    summary = {
        'stage': 'D16-P5K hard-cbar OCP residual training summary',
        ...
        'interpretation': 'P5K restores ASSB-style hard cbar/OCP baseline with bounded residual. Internal soft labels are not used during training.',
    }
    write_json(summary, out_dir / 'D16_P5K_TRAINING_SUMMARY.json')
    print('[D16-P5K train] wrote:', out_dir / 'D16_P5K_TRAINING_SUMMARY.json', flush=True)
