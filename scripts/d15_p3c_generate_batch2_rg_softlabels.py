...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='D15-P3C generate provisional Batch-2 3-cell P2Dlite-RG soft labels from replay profiles.')
    p.add_argument('--config', default='configs/d15_p3_batch2_applicability_config.json')
    p.add_argument('--representative-manifest-csv', required=True)
...
def _process_one(profile_row: Dict[str, str], output_root: Path, cfg: Dict[str, Any], prior: Dict[str, Any]) -> Dict[str, Any]:
    ...
    out.update({
        ...
        'radial_solver_version': np.array('D15-P3-Batch2-P2Dlite-RG-provisional-v1'),
        'radial_gradient_quality_flag': np.array('D15P3_GENERATED_NEEDS_AUDIT'),
        'source_profile_npz': np.array(str(npz_path)),
        'source_replay_profile_npz': np.array(str(npz_path)),
        'softlabel_boundary_note': np.array('Provisional Batch-2 model-consistent P2Dlite-RG labels; not experimental internal-state truth.'),
    })
...
def main() -> int:
    ...
    rows = read_csv_rows(args.representative_manifest_csv)
    summaries: List[Dict[str, Any]] = []
    errors: List[Dict[str, Any]] = []
    for idx, row in enumerate(rows, start=1):
        print(f'[D15-P3C RG generate] {idx}/{len(rows)} {row.get("profile_id")}', flush=True)
        try:
            ...
            print('[D15-P3C RG generate] ERROR:', errors[-1], flush=True)
    report = {
        'stage': 'D15-P3C Batch-2 3-cell P2Dlite-RG provisional soft-label generation',
        'output_dir': str(output_root),
        'representative_manifest_csv': str(args.representative_manifest_csv),
        'profile_count_requested': len(rows),
        'generated_count': len(summaries),
        'error_count': len(errors),
        'summaries': summaries,
        'errors': errors,
        'overall_status': 'PASS' if len(errors) == 0 and len(summaries) == len(rows) and len(rows) == 3 else ('REVIEW' if len(summaries) >= 1 else 'FAIL'),
    }
    write_json(report, output_root / 'D15_P3C_BATCH2_RG_GENERATION_REPORT.json')
    write_csv(summaries, output_root / 'D15_P3C_BATCH2_RG_GENERATION_REPORT.csv')
    print('[D15-P3C RG generate] overall_status:', report['overall_status'])
