#!/usr/bin/env python
"""Build D12-S1D balanced low/normal 6-profile scorecard from prediction.npz files.
...
- D12_S1D_run_metrics.csv
- D12_S1D_segment_metrics.csv
- D12_S1D_mode_summary.csv
- D12_S1D_candidate_decisions.csv
- D12_S1D_scorecard_summary.json
"""
...
def main() -> None:
    ...
    out_dir = Path(args.output_dir) if args.output_dir else root / "D12_S1D_scorecard"
    ...
    _write_csv(out_dir / "D12_S1D_run_metrics.csv", run_rows)
    _write_csv(out_dir / "D12_S1D_segment_metrics.csv", seg_rows)
    _write_csv(out_dir / "D12_S1D_mode_summary.csv", mode_rows)
    _write_csv(out_dir / "D12_S1D_candidate_decisions.csv", decisions)
    summary = {
        "ok": True,
        "runs_root": str(root),
    ...
    (out_dir / "D12_S1D_scorecard_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
