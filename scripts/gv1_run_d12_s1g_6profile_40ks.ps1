...
$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1g_p2d_highlocal_fast_6x40ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1g_p2d_highlocal_fast_6x40ks_scorecard"
...
$Preflight = [ordered]@{
  stage = "D12-S1G S1E-soft highlocal fast 6-profile 40ks preflight"
  ...
  Change = "S1E-soft low anchor + loss-side local high limiter; no prediction-side high suppression; no mainline overwrite"
}
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1G_6profile_40ks_preflight.json")
...
Write-Host "D12-S1G 6x40ks done. Scorecard: $ScorecardDir"
