...
def extract_npy_member(npz_path: Path, key: str, cache_root: Path) -> Path:
    ...
    try:
        ...
        raise FileNotFoundError(f'P5K-G eval cache/source file missing or path too long. npz={npz_path}; key={key}; cache_root={cache_root}; dst={dst}') from exc
    return dst
...
def feature_chunk(t: np.ndarray, I: np.ndarray, V: np.ndarray, s: int, e: int, stats: Dict[str, float], qn_full: np.ndarray, meta: Dict[str, Any] | None = None, cfg: Dict[str, Any] | None = None) -> np.ndarray:
    ...
    shift_a_arr = np.full(e - s, float(shift_a), dtype=np.float32)
    shift_c_arr = np.full(e - s, float(shift_c), dtype=np.float32)
    gate_arr = np.full(e - s, float(gate), dtype=np.float32)
    X = np.stack([
        ...
        shift_a_arr, shift_c_arr, gate_arr,
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5K-G v3 evaluate hard-cbar/OCP residual model on all55 against P2Dlite-RG soft labels. Soft labels are evaluation-only.')
    ap.add_argument('--manifest', required=True)
    ap.add_argument('--model-dir', required=True)
    ap.add_argument('--out-dir', required=True)
    ap.add_argument('--config', default='configs/d16_p5kg_rulev2_strict_gate_config.json')
    ...
    print(f'[D16-P5K-G eval v3] mmap_cache_root={cache_root}', flush=True)
    ...
    for row in rows:
        ...
        try:
            ...
            for s in range(0, n, int(args.chunk_size)):
                ...
                X = feature_chunk(t, I, V, s, e, stats, qn, meta, cfg)
                ...
                if s == 0 or e == n:
                    print(f"[D16-P5K-G eval v3] {meta['profile_id']}: chunk {s}:{e}/{n}", flush=True)
            ...
            print(f"[D16-P5K-G eval v3] FAIL {meta.get('profile_id')}: {repr(exc)}", flush=True)
    ...
    write_csv(metrics_rows, out_dir / 'D16_P5KG_METRICS_BY_PROFILE.csv')
    write_csv(aggregate_rows('split:'), out_dir / 'D16_P5KG_SPLIT_METRICS.csv')
    write_csv(aggregate_rows('batch:'), out_dir / 'D16_P5KG_BATCH_METRICS.csv')
    write_csv(aggregate_rows('protocol:'), out_dir / 'D16_P5KG_PROTOCOL_METRICS.csv')
    write_json(failures, out_dir / 'D16_P5KG_FAILURES.json')

    score = {
        'stage': 'D16-P5K-G hard-cbar/OCP residual evaluation',
        ...
        'notes': [
            ...
            'P5K-G uses hard cbar/OCP-style baseline plus bounded residual; it avoids P5G heuristic theta-gap targets.'
        ],
        'failures': failures,
    }
    write_json(score, out_dir / 'D16_P5KG_FINAL_SCORECARD.json')
    print('[D16-P5K-G eval v3] operational_status:', score['operational_status'], 'evaluated=', len(metrics_rows), 'failures=', len(failures), flush=True)
    print('[D16-P5K-G eval v3] wrote:', out_dir / 'D16_P5KG_FINAL_SCORECARD.json', flush=True)
