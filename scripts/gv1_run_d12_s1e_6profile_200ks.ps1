...
if (-not $Confirm200ks) {
  throw "D12-S1E 200ks is gated. Run 6x40ks first and pass -Confirm200ks only if D12_S1E_candidate_decisions.csv promotes a candidate."
}
Set-Location $ProjectRoot

$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1e_p2d_anchor_budget_6x200ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1e_p2d_anchor_budget_6x200ks_scorecard"
...
$Preflight = [ordered]@{
  stage = "D12-S1E low-anchor-budget 6-profile 200ks preflight"
...
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1E_6profile_200ks_preflight.json")
...
Write-Host "D12-S1E 6x200ks done. Scorecard: $ScorecardDir"
