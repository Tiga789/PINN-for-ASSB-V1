#!/usr/bin/env python
"""Prepare D11-S5C low-target correction amplitude repair commands.

This script only generates PowerShell scripts.  It does not launch training.
D11-S5C is a 6-profile / 40 ks repair smoke after D11-S5A showed that the
low-voltage gate was active but the low-tail correction became too weak under
previous candidates.  The goal is to test distinct low-target downward-amplitude
settings before any 200 ks confirmation.
"""
...
OPTIONAL_ARGS = {
    ...
    "voltage_tail_balance": ["--voltage_tail_balance", "--w_voltage_tail_balance"],
...
MODES: Dict[str, Dict[str, object]] = {
    "baseline_d951": {
        ...
        "optional": {},
        "description": "D9.5.1 reference; trend-first warmup rare-regime mainline.",
    },
    "lowtarget_amplify_down_1p25": {
        "profile_adaptive_mode": "trend_tail_hybrid",
        "rare_loss_warmup_start_frac": 0.72,
        "rare_loss_warmup_full_frac": 1.00,
        "rare_loss_start_scale": 0.00,
        "rare_loss_final_scale": 2.25,
        "rare_sample_start_scale": 0.08,
        "rare_sample_final_scale": 1.10,
        "optional": {
            "voltage_tail": 0.42,
            ...
            "high_tail_extra_gain": 0.4,
            "low_voltage_gate_center_V": 3.00,
            "low_voltage_gate_width_V": 0.22,
            "low_tail_correction_scale": 1.25,
            "low_tail_downward_gain": 1.25,
        },
        "description": "Mild amplitude repair: make low-tail/downward terms stronger than D11-S5A without hard clamp.",
    },
    "lowtarget_amplify_down_1p50": {
        "profile_adaptive_mode": "trend_tail_hybrid",
        "rare_loss_warmup_start_frac": 0.76,
        "rare_loss_warmup_full_frac": 1.00,
        "rare_loss_start_scale": 0.00,
        "rare_loss_final_scale": 2.80,
        "rare_sample_start_scale": 0.06,
        "rare_sample_final_scale": 1.25,
        "optional": {
            "voltage_tail": 0.55,
            "voltage_tail_balance": 0.08,
            "tail_fraction": 0.25,
            "tail_weight_gain": 2.6,
            "low_tail_extra_gain": 4.5,
            "high_tail_extra_gain": 0.35,
            "low_voltage_gate_center_V": 3.02,
            "low_voltage_gate_width_V": 0.24,
            "low_tail_correction_scale": 1.50,
            "low_tail_downward_gain": 1.50,
        },
        "description": "Medium amplitude repair: target stronger downward correction on target<=2.75 V.",
    },
    "lowtarget_amplify_down_1p75_guarded": {
        "profile_adaptive_mode": "trend_tail_hybrid",
        "rare_loss_warmup_start_frac": 0.82,
        "rare_loss_warmup_full_frac": 1.00,
        "rare_loss_start_scale": 0.00,
        "rare_loss_final_scale": 3.30,
        "rare_sample_start_scale": 0.05,
        "rare_sample_final_scale": 1.35,
        "optional": {
            "voltage_tail": 0.65,
            "voltage_tail_balance": 0.10,
            "tail_fraction": 0.26,
            "tail_weight_gain": 3.0,
            "low_tail_extra_gain": 5.5,
            "high_tail_extra_gain": 0.25,
            "low_voltage_gate_center_V": 3.05,
            "low_voltage_gate_width_V": 0.26,
            "direct_voltage_mix": 0.78,
            "low_tail_correction_scale": 1.75,
            "low_tail_downward_gain": 1.75,
        },
        "description": "Strong but guarded amplitude repair.  Promotion requires low_target improvement and no global/high-tail damage.",
    },
}
...
def build_train_command(
    ...
    if args_map.get("run_tag"):
        pieces += [args_map["run_tag"], f"D11-S5C_{mode_name}_{profile}"]
...
def main() -> None:
    ...
    command_dir = cache_root / "xjtu_batch134_d11_s5c_lowtarget_amplitude_repair_commands"
    output_root = cache_root / "xjtu_batch134_d11_s5c_lowtarget_amplitude_repair"
    ...
    manifest_path = command_dir / "d11_s5c_selected_lowtarget_focus_manifest.csv"
    ...
    for mode_name, mode_cfg in MODES.items():
        lines = [
            ...
            f"Write-Host '==== D11-S5C mode: {mode_name} ===='",
            "Write-Host 'Policy: 6 profiles, 40ks, battery-8 excluded, metadata_on disabled, hard clamp disabled.'",
        ]
        ...
        ps1 = command_dir / f"run_d11_s5c_{mode_name}.generated.ps1"
        write_ps1(ps1, "\n".join(lines) + "\n")
    ...
    for ps1 in all_scripts:
        all_lines += [f"& {ps_quote(str(ps1))}", "if ($LASTEXITCODE -ne 0) { throw 'D11-S5C generated mode script failed.' }"]
    all_ps1 = command_dir / "run_d11_s5c_all_modes.generated.ps1"
    write_ps1(all_ps1, "\n".join(all_lines) + "\n")

    summary = {
        "ok": True,
        "stage": "D11-S5C low-target correction amplitude repair command preparation",
        ...
        "promotion_policy": "Do not expand to 200 ks unless low_target and low_target_le_2p75 improve versus baseline without global/rest/high-tail damage.",
    }
    (command_dir / "d11_s5c_command_preparation_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
