#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Build D12-S2 balanced metadata ablation scorecard directly from prediction.npz.
...
def compute_prediction_metrics(prediction_npz: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    payload = {
        "ok": True,
        "stage": "D12-S2 voltage metrics computed from prediction.npz",
...
def mode_from_name(name: str) -> str:
    match = re.search(r"d12_s2_metadata_(off|zero|on)_", name)
    if match:
...
def profile_from_name(name: str) -> str:
    # New name: xjtu_batch134_d12_s2_metadata_on_Batch-1_2C_battery-1_TRUE_SMOKE_40ks_e100
...
def main() -> None:
    ...
    out_dir = Path(args.out_dir) if args.out_dir else cache_root / "xjtu_batch134_d12_s2_metadata_ablation_scorecard"

    run_dirs = sorted([
        p for p in cache_root.iterdir()
        if p.is_dir() and p.name.startswith("xjtu_batch134_d12_s2_metadata_") and "TRUE_SMOKE" in p.name
    ])
    ...
    for run_dir in run_dirs:
        ...
        if prediction is None:
            ...
            try:
                ...
                write_json(run_dir / "d12_s2_voltage_metrics.json", metrics_payload)
                write_csv(run_dir / "d12_s2_voltage_metrics_by_segment.csv", segs)
    ...
    if not rows:
        verdict = "d12_s2_no_run_dirs_found"
    elif completed == len(rows):
        verdict = "d12_s2_all_runs_completed_metrics_ok"
    elif completed > 0:
        verdict = "d12_s2_partial_runs_completed_review_missing"
    else:
        verdict = "d12_s2_no_completed_predictions"

    summary = {
        "ok": True,
        "stage": "D12-S2 balanced metadata ablation scorecard from predictions",
        "run_count": len(rows),
        "counts": counts,
    ...
    write_csv(out_dir / "d12_s2_scorecard.csv", rows)
    write_csv(out_dir / "d12_s2_segment_metrics.csv", segment_rows)
    write_csv(out_dir / "d12_s2_mode_summary.csv", mode_rows)
    write_json(out_dir / "d12_s2_scorecard_summary.json", summary)
    md_lines = [
        "# D12-S2 balanced metadata ablation scorecard",
        "",
        "## Verdict",
        "",
        f"```text\n{verdict}\n```",
    ...
    (out_dir / "D12_S2_SCORECARD_RECOMMENDATION.md").write_text("\n".join(md_lines), encoding="utf-8")
