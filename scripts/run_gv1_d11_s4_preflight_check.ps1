...
if ($AfterPrepare) {
  ...
  $cmdDir = Join-Path $CacheRoot "xjtu_batch134_d11_s4_lowtail_correction_smoke_commands"
  ...
  if ($bad) {
    ...
    throw "Unsafe generated command parameter detected. Stop before running D11-S4."
  }
  $summaryPath = Join-Path $cmdDir "d11_s4_command_preparation_summary.json"
  if (-not (Test-Path $summaryPath)) { throw "Missing preparation summary: $summaryPath" }
  Write-Host "Generated scripts look safe."
}

Write-Host "D11-S4 preflight PASS."
