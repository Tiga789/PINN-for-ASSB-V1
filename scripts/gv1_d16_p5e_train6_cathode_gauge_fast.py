...
def physics_observation_loss(raw: torch.Tensor, x: torch.Tensor, cfg: Dict[str, Any]) -> Tuple[torch.Tensor, Dict[str, float]]:
    """D16-P5E cathode-side offset-safe observation-physics loss.
    ...
    Difference from P5D:
      * keeps P5D's delta-gauge/correlation terms;
      * adds weak cathode-floor/anode-ceiling and offset-compensation guards derived from fixed
        prior ranges and I/V-derived feature trends;
      * avoids P5C-v1 hard absolute theta anchor that worsened theta gauge bias.
    """
    w = cfg['training'].get('loss_weights', {})
    p5e = cfg.get('p5e_cathode_gauge', {})
    grad_clip = float(cfg.get('model', {}).get('gradient_clip', 0.25))
    y = transform_outputs(raw, grad_clip=grad_clip)

    v_z = x[:, 8]        # standardized local voltage feature; used only as relative proxy
    I_norm = x[:, 4]
    absI_norm = x[:, 5]
    q_norm = x[:, 7]
    is_rest = x[:, 11]

    # Voltage observation anchor for phis_c; V(t) is an observed allowed signal.
    ...
    loss_v_corr = _corr_loss(y['theta_a_mean'], v_z, target_sign=+1.0) + _corr_loss(y['theta_c_mean'], v_z, target_sign=-1.0)

    # Centered two-electrode coupling. Suppresses paired drift of variations without imposing an exact absolute sum.
    ta = y['theta_a_mean']
    tc = y['theta_c_mean']
    ...
    # Weak midrange guard prevents pathological saturation.
    mid_margin = float(p5e.get('midrange_margin', 0.42))
    loss_midrange = torch.mean(torch.relu(torch.abs(ta - 0.5) - mid_margin) ** 2) + torch.mean(torch.relu(torch.abs(tc - 0.5) - mid_margin) ** 2)

    # P5E cathode-side offset-safe guards.
    # These are fixed-prior guards, not soft-label supervision. They target the P5D failure mode:
    # theta_a too high and theta_c too low while phis_c remains excellent.
    cathode_floor = float(p5e.get('cathode_floor', 0.22))
    anode_ceiling = float(p5e.get('anode_ceiling', 0.82))
    loss_cathode_floor = torch.mean(torch.relu(cathode_floor - tc) ** 2)
    loss_anode_ceiling = torch.mean(torch.relu(ta - anode_ceiling) ** 2)

    # Center prior uses voltage only as a relative coordinate. It is deliberately weak and bounded.
    amp = float(p5e.get('center_prior_amplitude', 0.22))
    slope = float(p5e.get('center_prior_slope', 0.75))
    vshape = torch.tanh(slope * v_z)
    target_a_center = torch.clamp(0.50 + amp * vshape, 0.05, 0.95)
    target_c_center = torch.clamp(0.52 - amp * vshape, 0.05, 0.95)
    loss_cathode_center_prior = torch.mean((tc - target_c_center.detach()) ** 2) + 0.35 * torch.mean((ta - target_a_center.detach()) ** 2)

    # Pair-sum slack: discourage extreme total inventory offsets but allow profile-dependent offset.
    pair_center = float(p5e.get('pair_sum_center', 1.02))
    pair_slack = float(p5e.get('pair_sum_slack', 0.22))
    pair_dev = torch.relu(torch.abs((ta + tc) - pair_center) - pair_slack)
    loss_pair_offset = torch.mean(pair_dev ** 2)

    # Current-driven radial gradient direction and rest relaxation.
    grad_target_scale = float(p5e.get('gradient_target_scale', 0.05))
    ...
    # Smoothness on outputs as weak stochastic-batch regularization.
    if raw.shape[0] > 2:
        loss_smooth = (
    ...
    total = (
        ...
        float(w.get('theta_q_correlation', 0.16)) * loss_q_corr +
        float(w.get('theta_voltage_correlation', 0.035)) * loss_v_corr +
        float(w.get('theta_centered_mass_coupling', 0.10)) * loss_centered_couple +
        float(w.get('theta_midrange_guard', 0.004)) * loss_midrange +
        float(w.get('cathode_floor_guard', 0.09)) * loss_cathode_floor +
        float(w.get('anode_ceiling_guard', 0.05)) * loss_anode_ceiling +
        float(w.get('theta_cathode_center_prior', 0.035)) * loss_cathode_center_prior +
        float(w.get('theta_pair_offset_compensation', 0.06)) * loss_pair_offset +
        float(w.get('gradient_anchor', 0.07)) * loss_grad_anchor +
    ...
    parts = {
        ...
        'loss_cathode_floor_guard': float(loss_cathode_floor.detach().cpu()),
        'loss_anode_ceiling_guard': float(loss_anode_ceiling.detach().cpu()),
        'loss_theta_cathode_center_prior': float(loss_cathode_center_prior.detach().cpu()),
        'loss_theta_pair_offset_compensation': float(loss_pair_offset.detach().cpu()),
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5E FAST train6 cathode-gauge physics model. No internal soft-label data loss is used.')
    ...
    for r in train_rows:
        ...
        print(f"[D16-P5E fast train] loaded observed-only {r['profile_id']}: n={n} train={Xtr.shape[0]} val={Xva.shape[0]}", flush=True)
    ...
    if args.warm_start_model_dir and not args.no_warm_start:
        ...
        if ck.exists():
            try:
                ...
                print(f"[D16-P5E train] warm-start loaded: {ck}; missing={len(missing)} unexpected={len(unexpected)}", flush=True)
            except Exception as exc:
                print(f"[D16-P5E train] WARNING: warm-start failed from {ck}: {exc}", flush=True)
        else:
            print(f"[D16-P5E train] warm-start requested but checkpoint not found under {wm}", flush=True)
    ...
    best_epoch = -1
    bad_validations = 0
    history_path = out_dir / 'D16_P5E_TRAINING_HISTORY.csv'
    with history_path.open('w', newline='', encoding='utf-8') as f:
        ...
        for ep in range(1, epochs + 1):
            ...
            if ep % max(1, val_every) == 0 or ep == 1:
                msg = f'[D16-P5E fast train] epoch={ep} train={train_loss:.6g}'
                if np.isfinite(val_loss):
                    msg += f' val={val_loss:.6g} best_epoch={best_epoch} best_val={best_val:.6g}'
                print(msg, flush=True)
            if bad_validations >= patience:
                print(f'[D16-P5E fast train] early stop at epoch={ep}; best_epoch={best_epoch} best_val={best_val:.6g}', flush=True)
                break

    summary = {
        'stage': 'D16-P5E FAST train6 cathode-gauge physics training',
        ...
        'notes': [
            ...
            'This P5E fast trainer keeps sampled X_train/X_val tensors on GPU and validates every val_every epochs.',
            'P5E adds offset-safe cathode-floor/anode-ceiling, centered pair coupling and q/V trend constraints derived only from I/V and fixed priors.'
        ]
    }
    write_json(summary, out_dir / 'D16_P5E_TRAINING_SUMMARY.json')
    write_json({'stage': 'D16-P5E train input audit', 'rows': audit_rows}, out_dir / 'D16_P5E_TRAIN_INPUT_AUDIT.json')
    print('[D16-P5E fast train] status:', summary['status'], 'checkpoint:', summary['checkpoint'], flush=True)
