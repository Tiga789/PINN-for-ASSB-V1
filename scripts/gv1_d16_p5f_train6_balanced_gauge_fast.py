...
def physics_observation_loss(raw: torch.Tensor, x: torch.Tensor, cfg: Dict[str, Any], teacher_raw: torch.Tensor | None = None) -> Tuple[torch.Tensor, Dict[str, float]]:
    """D16-P5F balanced anode-cathode gauge observation-physics loss.
    ...
    P5F objective:
      * keep P5D's theta_a advantage via optional warm-start teacher preservation;
      * keep P5E's cathode improvement using only weak fixed-prior gauge guards;
      * avoid P5C/P5E-style hard absolute theta anchor that over-pushed inventory offset.
    """
    w = cfg['training'].get('loss_weights', {})
    p5f = cfg.get('p5f_balanced_gauge', {})
    ...
    anode_mean_ceiling = float(p5f.get('anode_mean_ceiling', 0.70))
    cathode_mean_floor = float(p5f.get('cathode_mean_floor', 0.31))
    anode_point_ceiling = float(p5f.get('anode_point_ceiling', 0.86))
    cathode_point_floor = float(p5f.get('cathode_point_floor', 0.18))
    ...
    pair_center = float(p5f.get('pair_sum_center', 1.02))
    pair_slack = float(p5f.get('pair_sum_slack', 0.18))
    ...
    amp = float(p5f.get('center_prior_amplitude', 0.10))
    slope = float(p5f.get('center_prior_slope', 0.60))
    base_a = float(p5f.get('center_prior_base_a', 0.46))
    base_c = float(p5f.get('center_prior_base_c', 0.56))
    ...
    grad_target_scale = float(p5f.get('gradient_target_scale', 0.05))
    ...
    total = (
        ...
        float(w.get('teacher_anode_preservation', 0.050)) * loss_teacher_anode +
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5F FAST train6 balanced-gauge physics model. No internal soft-label data loss is used.')
    ...
    for r in train_rows:
        ...
        print(f"[D16-P5F fast train] loaded observed-only {r['profile_id']}: n={n} train={Xtr.shape[0]} val={Xva.shape[0]}", flush=True)
    ...
    if args.warm_start_model_dir and not args.no_warm_start:
        ...
        if ck.exists():
            try:
                ...
                print(f"[D16-P5F train] warm-start loaded: {ck}; missing={len(missing)} unexpected={len(unexpected)}", flush=True)
            except Exception as exc:
                print(f"[D16-P5F train] WARNING: warm-start failed from {ck}: {exc}", flush=True)
        else:
            print(f"[D16-P5F train] warm-start requested but checkpoint not found under {wm}", flush=True)

    if warm_start_used:
        teacher_model = copy.deepcopy(model).to(device).eval()
        for _p in teacher_model.parameters():
            _p.requires_grad_(False)
        print('[D16-P5F train] teacher preservation model frozen from warm-start checkpoint', flush=True)
    ...
    best_epoch = -1
    bad_validations = 0
    history_path = out_dir / 'D16_P5F_TRAINING_HISTORY.csv'
    with history_path.open('w', newline='', encoding='utf-8') as f:
        ...
        for ep in range(1, epochs + 1):
            ...
            if ep % max(1, val_every) == 0 or ep == 1:
                msg = f'[D16-P5F fast train] epoch={ep} train={train_loss:.6g}'
                if np.isfinite(val_loss):
                    msg += f' val={val_loss:.6g} best_epoch={best_epoch} best_val={best_val:.6g}'
                print(msg, flush=True)
            if bad_validations >= patience:
                print(f'[D16-P5F fast train] early stop at epoch={ep}; best_epoch={best_epoch} best_val={best_val:.6g}', flush=True)
                break

    summary = {
        'stage': 'D16-P5F FAST train6 balanced-gauge physics training',
        ...
        'notes': [
            ...
            'This P5F fast trainer keeps sampled X_train/X_val tensors on GPU and validates every val_every epochs.',
            'P5F adds offset-safe balanced-floor/anode-ceiling, centered pair coupling and q/V trend constraints derived only from I/V and fixed priors.'
        ]
    }
    write_json(summary, out_dir / 'D16_P5F_TRAINING_SUMMARY.json')
    write_json({'stage': 'D16-P5F train input audit', 'rows': audit_rows}, out_dir / 'D16_P5F_TRAIN_INPUT_AUDIT.json')
    print('[D16-P5F fast train] status:', summary['status'], 'checkpoint:', summary['checkpoint'], flush=True)
