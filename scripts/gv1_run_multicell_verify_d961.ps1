param(
  ...
  [string]$OutRoot = "E:\XJTU battery dataset\_gv1_cache\xjtu_batch134_train_conditioned_pinn_multicell_d961",
...
$selectionJson = Join-Path $OutRoot "selected_profiles_d961_${windowTag}.json"
$metricsJson = Join-Path $OutRoot "metrics_summary_d961_${windowTag}.json"
$scorecardJson = Join-Path $OutRoot "scorecard_d961_${windowTag}.json"

Write-Host "===== D9.6.1 profile selection ====="
...
foreach ($p in @($payload.profiles)) { Write-Host " - $($p.run_id) => $($p.solution_npz)" }

foreach ($p in @($payload.profiles)) {
  $runOut = Join-Path $OutRoot ("$($p.run_id)_${windowTag}")
  Write-Host ""
  Write-Host "===== Training $($p.run_id) | D9.6.1 $windowTag ====="
