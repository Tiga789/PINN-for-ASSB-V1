#!/usr/bin/env python
"""Prepare D11-S5A low-target sign/gate diagnosis commands.

This script generates PowerShell run scripts only. It does not launch training.
D11-S5A is intentionally a 6-profile / 40 ks diagnostic stage, not a 200 ks
confirmation and not a mainline replacement.  B1_2C battery-8 remains excluded.
"""
...
# D11-S5A modes are diagnostic settings.  They are deliberately kept at 40 ks
# and must not be promoted unless low_target segments improve, not merely global MAE.
MODES = {
    "baseline_d951": {
    ...
    "lowtarget_gate_probe": {
        "profile_adaptive_mode": "trend_tail_hybrid",
        "rare_loss_warmup_start_frac": 0.65,
        "rare_loss_warmup_full_frac": 0.98,
        "rare_loss_start_scale": 0.00,
        "rare_loss_final_scale": 0.45,
        "rare_sample_start_scale": 0.10,
        "rare_sample_final_scale": 0.55,
        "description": "Weak tail emphasis to inspect whether the low-voltage gate actually activates on target<=2.75 V.",
    },
    "lowtarget_downward_mild": {
        "profile_adaptive_mode": "trend_tail_hybrid",
        "rare_loss_warmup_start_frac": 0.70,
        "rare_loss_warmup_full_frac": 0.99,
        "rare_loss_start_scale": 0.00,
        "rare_loss_final_scale": 1.25,
        "rare_sample_start_scale": 0.10,
        "rare_sample_final_scale": 0.85,
        "description": "Mild diagnostic candidate; pass only if low_target bias decreases without global trend damage.",
    },
    "lowtarget_downward_strict": {
        "profile_adaptive_mode": "trend_tail_hybrid",
        "rare_loss_warmup_start_frac": 0.75,
        "rare_loss_warmup_full_frac": 1.00,
        "rare_loss_start_scale": 0.00,
        "rare_loss_final_scale": 1.75,
        "rare_sample_start_scale": 0.05,
        "rare_sample_final_scale": 1.00,
        "description": "Stronger diagnostic candidate; still no hard clamp, used to expose sign/gate failure modes.",
    },
}
...
def build_train_command(
    ...
    if args_map.get("run_tag"):
        pieces += [args_map["run_tag"], f"D11-S5A_{mode_name}_{profile}"]
    return " ".join(pieces)
...
def main() -> None:
    ...
    command_dir = cache_root / "xjtu_batch134_d11_s5a_lowtarget_sign_gate_commands"
    output_root = cache_root / "xjtu_batch134_d11_s5a_lowtarget_sign_gate_diagnosis"
    ...
    manifest_path = command_dir / "d11_s5a_selected_lowtarget_focus_manifest.csv"
    ...
    for mode_name, mode_cfg in MODES.items():
        lines = [
            ...
            f"Write-Host '==== D11-S5A mode: {mode_name} ===='",
        ]
        for p in selected:
            profile = p["profile"]
            out_dir = output_root / mode_name / profile
            cmd = build_train_command(
                args.python_exe, project_root, train_script, args_map,
        ...
        ps1 = command_dir / f"run_d11_s5a_{mode_name}.generated.ps1"
        write_ps1(ps1, "\n".join(lines) + "\n")
    ...
    for ps1 in all_scripts:
        all_lines += [f"& {ps_quote(str(ps1))}", "if ($LASTEXITCODE -ne 0) { throw 'D11-S5A generated mode script failed.' }"]
    all_ps1 = command_dir / "run_d11_s5a_all_modes.generated.ps1"
    write_ps1(all_ps1, "\n".join(all_lines) + "\n")

    summary = {
        "ok": True,
        "stage": "D11-S5A low-target sign/gate diagnosis command preparation",
        ...
        "promotion_policy": "Do not expand to 200 ks unless low_target and low_target_le_2p75 improve versus baseline.",
    }
    (command_dir / "d11_s5a_command_preparation_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
