...
def extract_npy_member(npz_path: Path, key: str, cache_root: Path) -> Path:
    ...
    rel = _safe_name(str(npz_path.parent).replace(':', ''))
    dst_dir = cache_root / rel
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5K evaluate hard-cbar/OCP residual model on all55 against P2Dlite-RG soft labels. Soft labels are evaluation-only.')
    ...
    for row in rows:
        ...
        try:
            ...
            for s in range(0, n, int(args.chunk_size)):
                ...
                if s == 0 or e == n:
                    print(f"[D16-P5K eval] {meta['profile_id']}: chunk {s}:{e}/{n}", flush=True)
            ...
            print(f"[D16-P5K eval] FAIL {meta.get('profile_id')}: {repr(exc)}", flush=True)
    ...
    print('[D16-P5K eval] operational_status:', score['operational_status'], 'evaluated=', len(metrics_rows), 'failures=', len(failures), flush=True)
    print('[D16-P5K eval] wrote:', out_dir / 'D16_P5K_FINAL_SCORECARD.json', flush=True)
