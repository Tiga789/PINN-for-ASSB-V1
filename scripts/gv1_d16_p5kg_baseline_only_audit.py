...
# Import the P5K-G evaluator as a local module so baseline math and exact-R2 accumulation stay identical.
import importlib.util

HERE = Path(__file__).resolve().parent
spec = importlib.util.spec_from_file_location("p5kg_eval", HERE / "gv1_d16_p5kg_eval55_vs_softlabels_v3.py")
p5kg = importlib.util.module_from_spec(spec)
assert spec.loader is not None
spec.loader.exec_module(p5kg)
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5K-G baseline-only exact-R2 preflight. No training and no checkpoint loading.')
    ...
    rows = p5kg.read_manifest(args.manifest)
    ...
    def add_group(group: str, metric: str, true: np.ndarray, pred: np.ndarray):
        group_acc.setdefault(group, {}).setdefault(metric, p5kg.Accum()).update(true, pred)

    for row in rows:
        meta = p5kg.parse_meta(row); npz_path = Path(row['softlabel_npz'])
        try:
            npz_path = p5kg.resolve_npz_path(npz_path, meta.get('profile_id',''), softlabel_root)
            arr = p5kg.load_mmap_arrays(npz_path, cache_root)
            t = p5kg.as_1d_float(arr['t']); I = p5kg.as_1d_float(arr['I']); V = p5kg.as_1d_float(arr['V']); n = len(t)
            ...
            accs = {m: p5kg.Accum() for m in ['theta_a_mean', 'theta_c_mean', 'theta_a', 'theta_c']}
            ...
            for s in range(0, n, int(args.chunk_size)):
                ...
                true_ta = p5kg.orient2d(arr['theta_a'], n, s, e); true_tc = p5kg.orient2d(arr['theta_c'], n, s, e)
                ...
                if s == 0 or e == n:
                    print(f"[D16-P5K-G baseline audit] {meta['profile_id']}: chunk {s}:{e}/{n}", flush=True)
            ...
            print(f"[D16-P5K-G baseline audit] FAIL {meta.get('profile_id')}: {repr(exc)}", flush=True)
    ...
    write_csv(metrics_rows, out_dir / 'D16_P5KG_BASELINE_ONLY_BY_PROFILE.csv')
    write_csv(aggregate('split:'), out_dir / 'D16_P5KG_BASELINE_ONLY_SPLIT_METRICS.csv')
    write_json(failures, out_dir / 'D16_P5KG_BASELINE_ONLY_FAILURES.json')
    score = {'stage': 'D16-P5K-G baseline-only preflight', 'manifest': str(args.manifest), 'config': str(args.config), 'profile_count_requested': len(rows), 'profile_count_evaluated': len(metrics_rows), 'failure_count': len(failures), 'operational_status': 'PASS' if len(metrics_rows) == len(rows) and not failures else 'REVIEW', 'global_metrics_weighted': all_row, 'notes': ['No training, no checkpoint loading. This computes the hard baseline with raw residuals set to zero.', 'Use this before long training to detect P5K-D-style bad baseline drift.']}
    write_json(score, out_dir / 'D16_P5KG_BASELINE_ONLY_SCORECARD.json')
    print('[D16-P5K-G baseline audit] operational_status:', score['operational_status'], 'evaluated=', len(metrics_rows), 'failures=', len(failures), flush=True)
    print('[D16-P5K-G baseline audit] wrote:', out_dir / 'D16_P5KG_BASELINE_ONLY_SCORECARD.json', flush=True)
