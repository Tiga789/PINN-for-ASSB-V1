param(
  ...
  [int]$Seed = 42,
  [string]$Device = "auto",
  [string[]]$Modes = @("baseline_d951", "d12s1d_p2d_low_regret_soft", "d12s1d_p2d_low_regret_mid", "d12s1d_p2d_low_regret_guarded"),
  [switch]$Clean,
  [switch]$Confirm200ks
)
...
if (-not $Confirm200ks) {
  throw "D12-S1D 200ks is gated. Run 6x40ks first and pass -Confirm200ks only if D12_S1D_candidate_decisions.csv promotes a candidate."
}
Set-Location $ProjectRoot

$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1d_p2d_regret_guarded_6x200ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1d_p2d_regret_guarded_6x200ks_scorecard"
...
function Get-ModeArgs([string]$Mode) {
  switch ($Mode) {
    ...
    "d12s1d_p2d_low_regret_soft" {
      # Closest to S1C soft, but with explicit normal/non-low regret guards.
      return @(
        "--p2d_transport_scale_V", "0.128",
        "--p2d_max_correction_V", "0.42",
        "--p2d_protocol_gain", "0.16",
        "--p2d_transport_gate_center_V", "3.10",
        "--p2d_transport_gate_width_V", "0.18",
        "--p2d_transport_pred_center_V", "3.53",
        "--p2d_transport_pred_width_V", "0.20",
        "--p2d_low_gate_power", "1.12",
        "--p2d_pred_low_gate_power", "1.18",
        "--p2d_normal_suppression_center_V", "0.0",
        "--p2d_lowtarget_focus_weight", "0.34",
        "--p2d_deep_coverage_weight", "0.105",
        "--p2d_normal_preservation_weight", "0.72",
        "--p2d_normal_down_bias_guard_weight", "7.0",
        "--p2d_normal_down_shift_guard_weight", "9.0",
        "--p2d_normal_down_allowed_shift_V", "0.006",
        "--p2d_normal_regret_guard_weight", "10.0",
        "--p2d_normal_regret_allowed_V", "0.0020",
        "--p2d_nonlow_regret_guard_weight", "4.0",
        "--p2d_nonlow_regret_allowed_V", "0.0030",
        "--p2d_rest_preservation_weight", "0.22",
        "--p2d_high_preservation_weight", "0.20",
        "--p2d_correction_l2_weight", "0.060",
        "--p2d_preservation_huber_delta_V", "0.042"
      )
    }
    "d12s1d_p2d_low_regret_mid" {
      # Intended promotion candidate: keeps S1C mid low gain but directly penalizes
      # normal MAE regret, the observed reason S1C missed promotion.
      return @(
        "--p2d_transport_scale_V", "0.142",
        "--p2d_max_correction_V", "0.46",
        "--p2d_protocol_gain", "0.18",
        "--p2d_transport_gate_center_V", "3.08",
        "--p2d_transport_gate_width_V", "0.17",
        "--p2d_transport_pred_center_V", "3.54",
        "--p2d_transport_pred_width_V", "0.20",
        "--p2d_low_gate_power", "1.18",
        "--p2d_pred_low_gate_power", "1.24",
        "--p2d_normal_suppression_center_V", "0.0",
        "--p2d_lowtarget_focus_weight", "0.42",
        "--p2d_deep_coverage_weight", "0.135",
        "--p2d_normal_preservation_weight", "0.86",
        "--p2d_normal_down_bias_guard_weight", "9.0",
        "--p2d_normal_down_shift_guard_weight", "12.0",
        "--p2d_normal_down_allowed_shift_V", "0.0055",
        "--p2d_normal_regret_guard_weight", "14.0",
        "--p2d_normal_regret_allowed_V", "0.0015",
        "--p2d_nonlow_regret_guard_weight", "6.0",
        "--p2d_nonlow_regret_allowed_V", "0.0025",
        "--p2d_rest_preservation_weight", "0.24",
        "--p2d_high_preservation_weight", "0.21",
        "--p2d_correction_l2_weight", "0.070",
        "--p2d_preservation_huber_delta_V", "0.040"
      )
    }
    "d12s1d_p2d_low_regret_guarded" {
      # Higher low drive, higher regret guard. If mid still leaks normal, this should
      # reveal whether low>=20mV and normal<=5mV can co-exist in the current branch.
      return @(
        "--p2d_transport_scale_V", "0.154",
        "--p2d_max_correction_V", "0.50",
        "--p2d_protocol_gain", "0.20",
        "--p2d_transport_gate_center_V", "3.06",
        "--p2d_transport_gate_width_V", "0.16",
        "--p2d_transport_pred_center_V", "3.55",
        "--p2d_transport_pred_width_V", "0.19",
        "--p2d_low_gate_power", "1.25",
        "--p2d_pred_low_gate_power", "1.30",
        "--p2d_normal_suppression_center_V", "0.0",
        "--p2d_lowtarget_focus_weight", "0.50",
        "--p2d_deep_coverage_weight", "0.160",
        "--p2d_normal_preservation_weight", "1.00",
        "--p2d_normal_down_bias_guard_weight", "11.0",
        "--p2d_normal_down_shift_guard_weight", "15.0",
        "--p2d_normal_down_allowed_shift_V", "0.005",
        "--p2d_normal_regret_guard_weight", "18.0",
        "--p2d_normal_regret_allowed_V", "0.0010",
        "--p2d_nonlow_regret_guard_weight", "8.0",
        "--p2d_nonlow_regret_allowed_V", "0.0020",
        "--p2d_rest_preservation_weight", "0.26",
        "--p2d_high_preservation_weight", "0.22",
        "--p2d_correction_l2_weight", "0.082",
...
$Preflight = [ordered]@{
  stage = "D12-S1D regret-guarded 6-profile 200ks preflight"
  ...
  Change = "normal/nonlow regret guards relative to baseline; no post-hoc correction; no mainline overwrite"
}
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1D_6profile_200ks_preflight.json")
...
& $PythonExe -m py_compile `
  ...
  (Join-Path $ProjectRoot "scripts\gv1_scorecard_d12_s1d.py")
...
& $PythonExe (Join-Path $ProjectRoot "scripts\gv1_scorecard_d12_s1d.py") `
  --runs_root $RunsRoot `
...
Write-Host "D12-S1D 6x200ks done. Scorecard: $ScorecardDir"
