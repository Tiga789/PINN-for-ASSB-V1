#!/usr/bin/env python
"""Build a D9.6.1 multi-profile scorecard from gv1_prediction_metrics.py output."""
...
def _row_status(row: dict[str, Any], window: str) -> tuple[str, list[str]]:
    ...
    # D9.6.1 is a verification step. We keep thresholds realistic for cross-cell checks,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Create D9.6.1 multi-profile scorecard.")
    ...
    if fail_count > 0:
        overall = "fail"
        recommendation = "暂停扩窗；先检查 fail profile 的协议/低压段/高压饱和，再考虑 D9.6.1.1 修正。"
        ...
        overall = "pass"
        recommendation = "可以进入 D9.6.1 下一步：6-profile 200ks；若当前已是 200ks，则再考虑 24-profile 40ks。"

    out = {
        "ok": bool(rows),
        "stage": "GV1 D9.6.1 multi-profile verification scorecard",
