...
def load_existing_prediction_metrics(pred_file: Path, slices: Mapping[str, Tuple[int,int]]) -> Dict[str, Any]:
    """Load metrics from either a full prediction NPZ or a compact v7 NPZ.

    v4/v6 full files contain y_true/y_pred arrays. v7 long-profile files are
    metrics-only compact markers to avoid >4 GB Windows/NumPy NPZ writes.
    """
    side = pred_file.with_suffix(".metrics.json")
    if side.exists():
        with side.open("r", encoding="utf-8") as f:
            row = json.load(f)
        row.setdefault("prediction_file", str(pred_file))
        row.setdefault("status", "EXISTING_SIDE_CAR")
        return row
    with np.load(pred_file, allow_pickle=True) as z:
        if "metrics_json" in z:
            raw = np.asarray(z["metrics_json"]).reshape(-1)[0]
            row = json.loads(str(raw))
            row.setdefault("prediction_file", str(pred_file))
            row.setdefault("status", "EXISTING_COMPACT")
            return row
    ...
    row = ps.as_dict()
    row.update({"profile_id": pid, "prediction_file": str(pred_file), "n_eval": int(yt.shape[0]), "status": "EXISTING_FULL_ARRAY"})
...
def infer_n_time_from_arrays(arrs: Mapping[str, np.ndarray]) -> int:
    """Infer the time dimension from theta arrays first, then t/I.

    Some very large XJTU profiles can contain auxiliary 2 x N arrays; using
    arr.reshape(-1) as n_time can double the true length and trigger invalid
    mmap/write shapes. Prefer the dimension paired with the radial dimension.
    """
    candidates: List[int] = []
    for key in ["theta_a", "theta_c"]:
        ...
        if a.ndim == 2:
            if min(a.shape) <= 256:
                candidates.append(int(max(a.shape)))
            else:
                candidates.append(int(a.shape[0]))
        elif a.ndim == 1:
            candidates.append(int(a.shape[0]))
    if candidates:
        # The two electrodes should agree; use the most common / smallest large value.
    ...
    if t.ndim == 2:
        return int(max(t.shape))
    return int(t.reshape(-1).shape[0])


def one_d_get_v7(arr: np.ndarray, start: int, end: int, n_time: int, name: str) -> np.ndarray:
    a = np.asarray(arr)
    if a.ndim == 0:
        return np.full(end - start, float(a), dtype=np.float32)
    if a.ndim == 1:
        if a.shape[0] == n_time:
            return np.asarray(a[start:end], dtype=np.float32)
        if a.shape[0] % n_time == 0:
            return np.asarray(a.reshape(-1, n_time)[-1, start:end], dtype=np.float32)
    elif a.ndim == 2:
        if a.shape[0] == n_time:
            # If there are multiple columns, use the last column as the most likely value channel.
            col = -1 if a.shape[1] > 1 else 0
            return np.asarray(a[start:end, col], dtype=np.float32)
        if a.shape[1] == n_time:
            row = -1 if a.shape[0] > 1 else 0
            return np.asarray(a[row, start:end], dtype=np.float32)
        if a.size % n_time == 0:
            return np.asarray(a.reshape(-1, n_time)[-1, start:end], dtype=np.float32)
    raise ValueError(f"{name}: cannot extract 1D series from shape {a.shape} for n_time={n_time}")


def streaming_mean_std_v7(arr: np.ndarray, n_time: int, chunk: int, fill_if_bad: float = 0.0) -> Tuple[float, float]:
    s = 0.0
    s2 = 0.0
    n = 0
    for st in range(0, n_time, chunk):
        ed = min(n_time, st + chunk)
        x = one_d_get_v7(arr, st, ed, n_time, "stat")
        m = np.isfinite(x)
        if np.any(m):
            xf = x[m].astype(np.float64)
            s += float(np.sum(xf))
...
def streaming_abs_percentile_v7(arr: np.ndarray, n_time: int, chunk: int, percentile: float = 99.5, sample_limit: int = 500_000) -> float:
    vals: List[np.ndarray] = []
    if n_time <= sample_limit:
        for st in range(0, n_time, chunk):
            ed = min(n_time, st + chunk)
            vals.append(np.abs(one_d_get_v7(arr, st, ed, n_time, "I")))
    else:
        step = max(1, n_time // sample_limit)
        # Sequential sampled reads avoid a huge fancy-index allocation.
        idx = np.arange(0, n_time, step, dtype=np.int64)[:sample_limit]
        for st in range(0, idx.size, 100_000):
            part = []
            for ii in idx[st:st+100_000]:
                part.append(one_d_get_v7(arr, int(ii), int(ii)+1, n_time, "I")[0])
            vals.append(np.abs(np.asarray(part, dtype=np.float32)))
    x = np.concatenate(vals) if vals else np.array([], dtype=np.float32)
    x = x[np.isfinite(x)]
    if x.size == 0:
        return 1.0
...
def q_scale_pass_v7(t_arr: np.ndarray, I_arr: np.ndarray, n_time: int, chunk: int) -> float:
    q = 0.0
    qmax = 0.0
    prev_t = float(one_d_get_v7(t_arr, 0, 1, n_time, "t")[0])
    for st in range(0, n_time, chunk):
        ed = min(n_time, st + chunk)
        t = one_d_get_v7(t_arr, st, ed, n_time, "t").astype(np.float64)
        I = one_d_get_v7(I_arr, st, ed, n_time, "I").astype(np.float64)
        if t.size == 0:
            continue
        dt = np.diff(t, prepend=prev_t)
        q_series = q + np.cumsum(I * dt) / 3600.0
...
def build_y_true_chunk_v7(arrs: Mapping[str, np.ndarray], start: int, end: int, n_time: int) -> np.ndarray:
    ta = orient_get(arrs["theta_a"], start, end, n_time, "theta_a")
    tc = orient_get(arrs["theta_c"], start, end, n_time, "theta_c")
    phie = one_d_get_v7(arrs["phie"], start, end, n_time, "phie").reshape(-1, 1)
    phis = one_d_get_v7(arrs["phis_c"], start, end, n_time, "phis_c").reshape(-1, 1)
    return np.concatenate([ta, tc, phie, phis], axis=1).astype(np.float32)
...
def infer_one_fullprofile(npz_path: Path, root: Path, pred_file: Path, model: Any, state: Mapping[str, Any], device: Any, profile_index: int, profile_count: int, primary_mode: str, batch_size: int, chunk_size: int, tmp_root: Path, compress_npz: bool = True) -> Dict[str, Any]:
    """Full-profile, chunked, metrics-first inference.

    Unlike v6, this does NOT create huge y_true/y_pred memmaps. That was the
    source of OSError(22) on 30M-point profiles. It still evaluates every time
    point, but stores only a compact marker NPZ plus a metrics sidecar.
    """
    ...
    try:
        ...
        n_time = infer_n_time_from_arrays(arrs)
        t_arr = arrs["t"]
        I_arr = arrs["I"]
        voltage_arr = arrs.get("voltage")
        temp_arr = arrs.get("temp")
        if voltage_arr is None:
            voltage_arr = arrs["phis_c"]
        if temp_arr is None:
            temp_arr = np.array(25.0, dtype=np.float32)
        t_first = float(one_d_get_v7(t_arr, 0, 1, n_time, "t")[0])
        t_last = float(one_d_get_v7(t_arr, n_time-1, n_time, n_time, "t")[0]) if n_time > 0 else t_first
        span = t_last - t_first
        if not np.isfinite(span) or span <= 0:
            span = 1.0
        I_scale = streaming_abs_percentile_v7(I_arr, n_time, chunk_size)
        vmean, vstd = streaming_mean_std_v7(voltage_arr, n_time, chunk_size, fill_if_bad=0.0)
        tempmean, tempstd = streaming_mean_std_v7(temp_arr, n_time, chunk_size, fill_if_bad=25.0)
        qscale = q_scale_pass_v7(t_arr, I_arr, n_time, chunk_size)
        ...
        for st in range(0, n_time, chunk_size):
            ...
            t = one_d_get_v7(t_arr, st, ed, n_time, "t")
            I = one_d_get_v7(I_arr, st, ed, n_time, "I")
            V = one_d_get_v7(voltage_arr, st, ed, n_time, "voltage")
            T = one_d_get_v7(temp_arr, st, ed, n_time, "temp")
            ...
            if q_series.size:
                q = float(q_series[-1])
                prev_t = float(t[-1])
            X, fnames = make_features_chunk(t, I, V, T, qn, t_first, span, I_scale, vmean, vstd, tempmean, tempstd, profile_index, profile_count, include_onehot)
            if feature_names_saved is None:
                feature_names_saved = fnames
            y_true = build_y_true_chunk_v7(arrs, st, ed, n_time)
            ...
            print(f"[D16-P5A v7] {pid}: metrics chunk {st}:{ed}/{n_time}", flush=True)
        row = stats.as_dict()
        b, pr, cell = parse_batch_protocol(pid)
        row.update({
            "profile_id": pid,
            ...
            "cell_key": cell,
            "n_eval": int(n_time),
            "status": "GENERATED_COMPACT_FULL_PROFILE_METRICS",
            "prediction_mode": primary_mode,
            "compact_prediction_npz": True,
            "full_profile_evaluated": True,
            "sampled": False,
            "runtime_s": float(time.time() - t_start_wall),
        })
        row["theta_total_count"] = int(row.get("theta_a_count", 0) or 0) + int(row.get("theta_c_count", 0) or 0)
        sidecar = pred_file.with_suffix(".metrics.json")
        write_json(row, sidecar)
        # Compact marker NPZ in the same predictions directory. This keeps file
        # accounting simple without writing impossible 4-10 GB single-NPZ arrays.
        pred_file.parent.mkdir(parents=True, exist_ok=True)
        np.savez_compressed(
            pred_file,
            profile_id=np.array(pid),
            n_eval=np.array(int(n_time), dtype=np.int64),
            prediction_mode=np.array(primary_mode),
            compact_full_profile_metrics=np.array(True),
            sampled=np.array(False),
            target_names=np.array(state.get("target_names", []), dtype=object),
            feature_names=np.array(feature_names_saved or state.get("feature_names", []), dtype=object),
            metrics_json=np.array(json.dumps(row, ensure_ascii=False)),
        )
        return row
    finally:
        shutil.rmtree(tmp_dir, ignore_errors=True)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="D16-P5A v7: fill missing ALL55 full-profile predictions using chunked inference.")
...
def main() -> int:
    ...
    tmp_root = run_dir / "_tmp_v7_fullprofile"
    ...
    print(f"[D16-P5A v7] soft profiles discovered={len(files)} existing metrics={len(metric_rows)} missing_to_generate={len(to_generate)}", flush=True)
    for npz_path, pid, pred_file, idx, reason in to_generate:
        try:
            print(f"[D16-P5A v7] GENERATE {pid} -> {pred_file}", flush=True)
    ...
    final = {
        "stage": "D16-P5A v7 full-profile fill-missing transfer evaluation",
        ...
        "notes": [
            "This is full-profile chunked inference. It fills missing profiles without rerunning already successful profiles.",
            "Only eval_full_profiles/predictions is written; raw/projected duplicate directories are intentionally not created. Long generated profiles are compact metrics NPZ markers plus .metrics.json sidecars, not huge y_true/y_pred dumps.",
            "primary_mode=projected clips theta_a/theta_c to the D15-P3B physical range but leaves phie/phis_c unchanged.",
        ],
        "failures": failures,
    }
    write_json(final, run_dir / "D16_P5A_V7_FINAL_SCORECARD.json")
    print("[D16-P5A v7] operational_status:", final["operational_status"], flush=True)
    print("[D16-P5A v7] prediction_file_count_primary:", pred_count, flush=True)
    print("[D16-P5A v7] wrote:", run_dir / "D16_P5A_V7_FINAL_SCORECARD.json", flush=True)
