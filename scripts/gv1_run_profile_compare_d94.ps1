param(
  ...
  [string]$OutRoot = "E:\XJTU battery dataset\_gv1_cache\xjtu_batch134_train_conditioned_pinn_profile_compare_d94",
  [int]$Epochs = 1200,
  [int]$BatchSize = 4096,
...
foreach ($r in $runs) {
  Write-Host ""
  Write-Host "===== Training $($r.name) | D9.4 profile-adaptive=$ProfileAdaptiveMode ====="
  Write-Host $r.path
...
& $Python .\scripts\gv1_prediction_metrics.py --root $OutRoot --output_json (Join-Path $OutRoot "metrics_summary_d94_${windowTag}.json")
