param(
  ...
  [int]$Seed = 42,
  [string]$Device = "auto",
  [string[]]$Modes = @("baseline_d951", "d12s1b_p2d_preserve_mid"),
  [switch]$Clean,
  [switch]$Confirm200ks
)
...
if (-not $Confirm200ks) {
  throw "D12-S1B 200ks is gated. Run 6x40ks first and pass -Confirm200ks only if D12_S1B_candidate_decisions.csv promotes a candidate."
}
Set-Location $ProjectRoot

$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1b_p2d_preservation_6x200ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1b_p2d_preservation_6x200ks_scorecard"
...
$Preflight = [ordered]@{
  stage = "D12-S1B preservation-tightened 6-profile 200ks preflight"
...
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1B_6profile_200ks_preflight.json")
...
Write-Host "D12-S1B 6x200ks done. Scorecard: $ScorecardDir"
