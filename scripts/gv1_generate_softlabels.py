...
from gv1.pipeline.data_loader import read_standard_tables_from_index
...
def _capacity_normalized_baseline(profile, q_ref_Ah: float | None) -> dict[str, np.ndarray]:
    """Lightweight cbar-like baseline for early GV1 plumbing.

    This is not the final electrochemical soft label. It allows the downstream
    pipeline to test measured-current replay before the full effective-SPM
...
def _profile_group_columns(df: pd.DataFrame) -> list[str]:
    # One .mat file usually corresponds to one cell. Group by source_file first
    # so multiple cells/batches never become one artificial time profile.
    for cols in [
        ['source_file'],
...
def main() -> None:
    ...
    ap.add_argument('--write_standard_csv', action='store_true')
    args = ap.parse_args()
    ...
    df = read_standard_tables_from_index(
        index_csv,
        adapter=adapter,
    ...
    # Preserve source row order. Do not globally sort by time, because some raw
    # files use step-relative time that resets within a cycle.
    df = df.reset_index(drop=True)

    if args.write_standard_csv:
        df.to_csv(out_dir / 'standard_table.csv', index=False, encoding='utf-8-sig')
    try:
        df.to_parquet(out_dir / 'standard_table.parquet', index=False)
    except Exception as exc:
        (out_dir / 'standard_table.parquet.failed.txt').write_text(str(exc), encoding='utf-8')
    ...
    manifest_rows = []
    first_npz = None
    for idx, (keys, g) in enumerate(groups, start=1):
        if not isinstance(keys, tuple):
            keys = (keys,)
        meta = dict(zip(group_cols, keys)) if group_cols else {'profile_id': 'all'}
        cell = _safe_name(meta.get('cell_id') or meta.get('battery_id') or f'profile_{idx:04d}')
        source_name = _safe_name(Path(str(meta.get('source_file', f'profile_{idx:04d}'))).stem)
        profile_id = f'{idx:04d}_{cell}_{source_name}'
        ...
        manifest_rows.append({
            ...
            'ok': audit.ok,
            'n_points': len(profile.t_s),
            **{k: str(v) for k, v in meta.items()},
        })
        if first_npz is None:
            first_npz = npz_path
            # Convenience copy for smoke scripts.
    ...
    cycle_summary.to_csv(out_dir / 'cycle_integrals.csv', index=False, encoding='utf-8-sig')
    ...
    print(json.dumps({'ok': ok_all, 'output_dir': str(out_dir), 'profile_count': len(manifest_rows), 'first_npz': str(first_npz) if first_npz else None}, ensure_ascii=False, indent=2))
