param(
  ...
  [string]$OutRoot = "E:\XJTU battery dataset\_gv1_cache\xjtu_batch134_train_conditioned_pinn_multicell_d962",
...
$selectionJson = Join-Path $OutRoot "selected_profiles_d962_${windowTag}.json"
$metricsJson = Join-Path $OutRoot "metrics_summary_d962_${windowTag}.json"
$scorecardJson = Join-Path $OutRoot "scorecard_d962_${windowTag}.json"

Write-Host "===== D9.6.2 profile selection ====="
...
foreach ($p in @($payload.profiles)) {
  ...
  Write-Host ""
  Write-Host "===== Training $($p.run_id) | D9.6.2 $windowTag | rollback trend-first core ====="
...
Write-Host ""
Write-Host "===== Collecting D9.6.2 metrics ====="
& $Python .\scripts\gv1_prediction_metrics.py --root $OutRoot --output_json $metricsJson

Write-Host ""
Write-Host "===== Building D9.6.2 scorecard ====="
& $Python .\scripts\gv1_multicell_scorecard_d962.py --metrics_json $metricsJson --output_json $scorecardJson
