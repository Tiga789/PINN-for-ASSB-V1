...
def main() -> int:
    ap = argparse.ArgumentParser(description='Build D16-P5K-D train6/train8/train10 manifest from ALL55 P2Dlite-RG soft-label directory.')
    ...
    if len(rows) != 55:
        print(f'[D16-P5K-D manifest] WARNING: expected 55 profiles, found {len(rows)}', flush=True)
    ...
    # Hard probes remain held-out/eval but are explicitly marked for later diagnosis.
    hard_probes = cfg.get('hard_probe_cells', [])
    hard_probe_map = {(str(x['batch']), str(x['battery'])): str(x.get('reason', 'hard_probe_eval')) for x in hard_probes}
    for r in rows:
        key = (r['batch'], r['battery'])
        if r.get('split') != 'train' and key in hard_probe_map:
            r['reason'] = hard_probe_map[key]
    ...
    report = {
        'stage': 'D16-P5K-D train6to10 manifest',
        ...
        'notes': [
            ...
            'P5K-D uses generator-aligned balanced train cells and keeps hard probes held out unless explicitly selected.'
        ]
    }
    with out_json.open('w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print('[D16-P5K-D manifest] status:', report['status'], 'train_set=', train_set, 'train_count=', train_count, 'eval_count=', eval_count, flush=True)
    print('[D16-P5K-D manifest] wrote:', out_csv, out_json, flush=True)
