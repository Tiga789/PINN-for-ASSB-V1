param(
  ...
  [string]$OutRoot = "E:\XJTU battery dataset\_gv1_cache\xjtu_batch134_train_conditioned_pinn_borderline_B1_2C_battery8_200ks_d961",
  [string]$Device = "cuda"
)
...
Write-Host "===== D9.6.1 targeted repair: B1_2C battery-8 200ks ====="
Write-Host $SolutionNpz

& $Python .\scripts\gv1_train_conditioned_pinn.py `
  --solution_npz $SolutionNpz `
  --output_dir $OutRoot `
  --profile_adaptive_mode late_2c_guarded `
  --epochs 1600 `
  --batch_size 4096 `
...
$metricsJson = Join-Path $OutRoot "metrics_borderline_200ks_d961.json"
