...
if ([string]::IsNullOrWhiteSpace($OutputRunsRoot)) {
  $OutputRunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1i_s1e_soft_highfallback_6x200ks"
}
if ([string]::IsNullOrWhiteSpace($ScorecardDir)) {
  $ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1i_s1e_soft_highfallback_6x200ks_scorecard"
}

$preflight = [ordered]@{
  stage = "D12-S1I high-region local wrapper 6-profile 200ks"
...
Write-Host "D12-S1I 200ks wrapper done. Scorecard: $ScorecardDir"
