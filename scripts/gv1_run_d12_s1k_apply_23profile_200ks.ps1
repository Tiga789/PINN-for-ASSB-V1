...
if ([string]::IsNullOrWhiteSpace($SourceRunsRoot)) {
  $SourceRunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1e_p2d_anchor_budget_23x200ks"
}
$OutputRunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1k_two_candidate_23x200ks_wrapper"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1k_two_candidate_23x200ks_scorecard"

$Preflight = [ordered]@{
  stage = "D12-S1K two-candidate 23-profile 200ks wrapper confirmation"
...
if (-not (Test-Path -LiteralPath $SourceRunsRoot -PathType Container)) {
  throw "SourceRunsRoot not found: $SourceRunsRoot. Generate S1E-soft + baseline source predictions first, or pass -SourceRunsRoot to an existing source prediction root."
}
...
& $PythonExe @cmd
if ($LASTEXITCODE -ne 0) { throw "D12-S1K 23-profile 200ks wrapper failed" }
Write-Host "D12-S1K 23-profile 200ks done. Scorecard: $ScorecardDir"
