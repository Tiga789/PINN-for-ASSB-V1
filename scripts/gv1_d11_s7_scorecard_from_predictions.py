#!/usr/bin/env python
"""Collect D11-S7 low-voltage escape scorecard from predictions.

No training is launched.  This script reads D11-S7 prediction.npz files and
checks whether the low-voltage escape candidates actually improve low_target and
...
def main() -> None:
    ...
    pred_root = cache_root / "xjtu_batch134_d11_s7_lowvoltage_escape"
    out_dir = cache_root / "xjtu_batch134_d11_s7_lowvoltage_escape_scorecard"
    ...
    write_csv(out_dir / "D11_S7_run_metrics.csv", run_rows)
    write_csv(out_dir / "D11_S7_segment_metrics.csv", seg_rows)
    write_csv(out_dir / "D11_S7_mode_summary.csv", mode_summary)
    write_csv(out_dir / "D11_S7_mode_segment_summary.csv", mode_seg_summary)
    write_csv(out_dir / "D11_S7_protocol_summary.csv", protocol_summary)
    write_csv(out_dir / "D11_S7_component_summary.csv", comp_summary)
    ...
    write_csv(out_dir / "D11_S7_global_vs_lowtarget_tradeoff.csv", compare_rows)
    ...
    next_action = "run_6profile_200ks_confirmation_for_promoted_escape_candidate" if promoted else "do_not_expand_redesign_lowvoltage_escape_or_model_capacity"
    verdict = "d11_s7_all_runs_completed_metrics_ok" if ok_runs == expected else "d11_s7_incomplete_or_failed_runs"
    summary = {
        "ok": ok_runs == expected,
        "stage": "D11-S7 low-target correction low-voltage escape scorecard from predictions",
    ...
    (out_dir / "D11_S7_scorecard_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")

    lines = [
        "# D11-S7 low-target correction low-voltage escape recommendation",
        "",
        f"- Verdict: `{verdict}`",
    ...
    if promoted:
        ...
        lines.append("No candidate passed the low-target criterion. Do not expand to 200 ks; redesign low-target escape head or consider protocol-specific/P2D-like correction.")
    lines += ["", "## Files", "", "- `D11_S7_mode_segment_summary.csv`", "- `D11_S7_global_vs_lowtarget_tradeoff.csv`", "- `D11_S7_component_summary.csv`", "- `D11_S7_scorecard_summary.json`"]
    (out_dir / "D11_S7_RECOMMENDATION.md").write_text("\n".join(lines), encoding="utf-8")
