...
def main() -> int:
    ap = argparse.ArgumentParser(description='Build D16-P5K-F train6/train8/train10 manifest from ALL55 P2Dlite-RG soft-label directory.')
    ...
    ap.add_argument('--train-set', default='', help='F_train12_profile_theta0 or C_train10_compat. Defaults to config default_train_set.')
    ...
    if len(rows) != 55:
        print(f'[D16-P5K-F manifest] WARNING: expected 55 profiles, found {len(rows)}', flush=True)
    ...
    report = {
        'stage': 'D16-P5K-F train12 profile-theta0 manifest',
        ...
        'notes': [
            ...
            'P5K-F train12 separates core_train and hard_probe; hard-probe cells are trained but reported separately, while the remaining cells stay held-out eval.'
        ]
    }
    with out_json.open('w', encoding='utf-8') as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    print('[D16-P5K-F manifest] status:', report['status'], 'train_set=', train_set, 'train_count=', train_count, 'eval_count=', eval_count, flush=True)
    print('[D16-P5K-F manifest] wrote:', out_csv, out_json, flush=True)
