param(
  ...
  [int]$Seed = 42,
  [string]$Device = "auto",
  [string[]]$Modes = @("baseline_d951", "d12s1f_p2d_low_anchor_highsafe_soft"),
  [switch]$Clean,
  [switch]$Confirm200ks
)
...
if (-not $Confirm200ks) {
  throw "D12-S1F 200ks is gated. Run 6x40ks first and pass -Confirm200ks only if D12_S1F_candidate_decisions.csv promotes a candidate."
}
Set-Location $ProjectRoot

$RunsRoot = Join-Path $CacheRoot "xjtu_batch134_d12_s1f_p2d_highsafe_fast_6x200ks"
$ProfilesRoot = Join-Path $CacheRoot "xjtu_batch134_replay_profiles"
$ScorecardDir = Join-Path $CacheRoot "xjtu_batch134_d12_s1f_p2d_highsafe_fast_6x200ks_scorecard"
...
function Get-ModeArgs([string]$Mode) {
  switch ($Mode) {
    ...
    "d12s1f_p2d_low_anchor_highsafe_soft" {
      # S1F-soft: S1E-soft was one flag from promotion.  Keep the low anchor,
      # lock high-voltage leakage, and disallow upward P2D correction that caused
      # pred>4.35 overshoot.  This is the primary fast-screen candidate.
      return @(
    ...
    "d12s1f_p2d_low_anchor_highsafe_mid" {
      # S1F-mid: slightly stronger low anchor than soft, but with the same high lock.
      # Use only if the soft candidate still under-corrects low_target.
      return @(
...
$Preflight = [ordered]@{
  stage = "D12-S1F low-highsafe-fast 6-profile 200ks preflight"
...
$Preflight | ConvertTo-Json -Depth 6 | Tee-Object -FilePath (Join-Path $RunsRoot "D12_S1F_6profile_200ks_preflight.json")
...
Write-Host "D12-S1F 6x200ks done. Scorecard: $ScorecardDir"
