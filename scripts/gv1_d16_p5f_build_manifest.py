...
def main() -> int:
    ap = argparse.ArgumentParser(description='Build D16-P5F train6/eval49 manifest from ALL55 P2Dlite-RG soft-label directory.')
    ...
    if len(rows) != 55:
        print(f'[D16-P5F manifest] WARNING: expected 55 profiles, found {len(rows)}', flush=True)
    ...
    report = {
        'stage': 'D16-P5F train6/eval49 manifest',
    ...
    print('[D16-P5F manifest] status:', report['status'], 'train_count=', train_count, 'eval_count=', eval_count, flush=True)
    print('[D16-P5F manifest] wrote:', out_csv, out_json, flush=True)
