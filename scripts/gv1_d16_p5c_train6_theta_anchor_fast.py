...
def physics_observation_loss(raw: torch.Tensor, x: torch.Tensor, cfg: Dict[str, Any]) -> Tuple[torch.Tensor, Dict[str, float]]:
    """Observation-physics loss for D16-P5C.

    Training boundary: this loss uses only network outputs and observed-feature columns derived
    from t_global_s / I_profile / voltage_exp. It does not use theta/cs/phie/phis_c soft labels.

    Compared with P5B, P5C adds stronger theta anchors:
      - OCP-inverse heuristic anchor from observed voltage level;
      - Coulomb-integral anchor from observed current integral q_norm;
      - two-electrode mass-coupling gauge theta_a + theta_c ~= 1;
      - gradient sign/magnitude prior from current direction and magnitude.
    These are still physics/observation constraints, not internal-state data loss.
    """
    w = cfg['training'].get('loss_weights', {})
    p5c = cfg.get('p5c_theta_anchor', {})
    ...
    # Voltage observation anchor for phis_c. This is the observed terminal-voltage time series,
    # which is allowed by the train6/eval49 boundary.
    loss_v = torch.mean((y['phis_c_norm'] - v_norm) ** 2)

    # OCP-inverse style theta gauge from voltage: high cell voltage means high cell SOC;
    # graphite/anode lithiation is high and cathode/NCM lithiation is low.
    ocp_gain = float(p5c.get('ocp_inverse_gain', 1.75))
    v_soc = torch.sigmoid(ocp_gain * v_norm)

    # Coulomb integral anchor from q_norm. q_norm is computed only from I(t)dt.
    q_soc = torch.clamp(0.5 + 0.5 * q_norm, 0.0, 1.0)
    voltage_blend = float(p5c.get('voltage_anchor_blend', 0.65))
    theta_a_anchor = torch.clamp(voltage_blend * v_soc + (1.0 - voltage_blend) * q_soc, 0.0, 1.0)
    theta_c_anchor = 1.0 - theta_a_anchor

    # Robust Huber-like penalty for theta gauge. This is not soft-label supervision; targets come from V/I only.
    beta = float(p5c.get('theta_anchor_huber_beta', 0.08))
    def smooth_l1(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        e = torch.abs(a - b)
        return torch.mean(torch.where(e < beta, 0.5 * e * e / max(beta, 1e-8), e - 0.5 * beta))

    loss_ocp_inverse = smooth_l1(y['theta_a_mean'], theta_a_anchor) + smooth_l1(y['theta_c_mean'], theta_c_anchor)

    # Two-electrode lithium inventory gauge. In normalized P2Dlite-RG labels the exact constant is not known
    # to the trainer, but theta_a + theta_c ~= 1 is a useful fixed-gauge prior that suppresses paired drift.
    mass_sum_target = float(p5c.get('mass_sum_target', 1.0))
    loss_mass_gauge = torch.mean((y['theta_a_mean'] + y['theta_c_mean'] - mass_sum_target) ** 2)

    # Current-driven gradient prior: charge I>0 enriches graphite surface and depletes cathode surface.
    grad_target_scale = float(p5c.get('gradient_target_scale', 0.08))
    ...
    # Rest relaxation: during rest, radial gradients should be small.
    loss_rest_relax = torch.mean(is_rest * (y['grad_a'] ** 2 + y['grad_c'] ** 2))

    # Existing weak regularizers retained.
    if raw.shape[0] > 2:
    ...
    total = (
        ...
        float(w.get('ocp_inverse_theta_anchor', 0.55)) * loss_ocp_inverse +
        float(w.get('coulomb_mass_gauge', 0.35)) * loss_mass_gauge +
        float(w.get('gradient_anchor', 0.18)) * loss_grad_anchor +
        float(w.get('rest_relaxation', 0.08)) * loss_rest_relax +
        float(w.get('theta_monotonic_sign', 0.10)) * loss_sign +
        float(w.get('theta_mass_coupling', 0.05)) * loss_couple +
        float(w.get('gradient_direction', 0.05)) * loss_grad_dir +
        float(w.get('smoothness', 0.012)) * loss_smooth +
        float(w.get('phie_regularization', 0.005)) * loss_phie
    )
    parts = {
        'loss_voltage_observation': float(loss_v.detach().cpu()),
        'loss_ocp_inverse_theta_anchor': float(loss_ocp_inverse.detach().cpu()),
        'loss_coulomb_mass_gauge': float(loss_mass_gauge.detach().cpu()),
...
def main() -> int:
    ap = argparse.ArgumentParser(description='D16-P5C FAST train6 theta-anchor physics model. No internal soft-label data loss is used.')
    ...
    for r in train_rows:
        ...
        print(f"[D16-P5C fast train] loaded observed-only {r['profile_id']}: n={n} train={Xtr.shape[0]} val={Xva.shape[0]}", flush=True)
    ...
    best_epoch = -1
    bad_validations = 0
    history_path = out_dir / 'D16_P5C_TRAINING_HISTORY.csv'
    with history_path.open('w', newline='', encoding='utf-8') as f:
        ...
        for ep in range(1, epochs + 1):
            ...
            if ep % max(1, val_every) == 0 or ep == 1:
                msg = f'[D16-P5C fast train] epoch={ep} train={train_loss:.6g}'
                if np.isfinite(val_loss):
                    msg += f' val={val_loss:.6g} best_epoch={best_epoch} best_val={best_val:.6g}'
                print(msg, flush=True)
            if bad_validations >= patience:
                print(f'[D16-P5C fast train] early stop at epoch={ep}; best_epoch={best_epoch} best_val={best_val:.6g}', flush=True)
                break

    summary = {
        'stage': 'D16-P5C FAST train6 theta-anchor physics training',
        ...
        'notes': [
            ...
            'This P5C fast trainer keeps sampled X_train/X_val tensors on GPU and validates every val_every epochs.',
            'P5C adds OCP-inverse, Coulomb/mass gauge and rest-relaxation constraints derived only from I/V.'
        ]
    }
    write_json(summary, out_dir / 'D16_P5C_TRAINING_SUMMARY.json')
    write_json({'stage': 'D16-P5C train input audit', 'rows': audit_rows}, out_dir / 'D16_P5C_TRAIN_INPUT_AUDIT.json')
    print('[D16-P5C fast train] status:', summary['status'], 'checkpoint:', summary['checkpoint'], flush=True)
