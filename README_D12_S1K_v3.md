...
This v3 package fixes the 23-profile source-generation parallel runner. The v2 source generator could fail when Windows paths contained spaces, because Start-Process received an unquoted argument list. v3 quotes every subprocess argument and treats `prediction.npz` as the required success artifact when PowerShell returns a null process exit code.
