...
# -*- coding: utf-8 -*-
"""
ModelFin_103 cycle-range soft-label evaluator.

This evaluator compares PINN predictions against soft-label arrays in one
continuous solution.npz, but only on a selected cycle interval.  It is intended
for evaluating ModelFin_103 directly on cycle 5-100 without training a new ID.

Important conventions:
- The model is evaluated as one reusable surrogate; cycle filtering is only a
  metric/sampling mask, not a retraining step.
- The original t_global_s values are kept after filtering.  Time is not reset at
  cycle 5 or at cycle boundaries.
- voltage_exp/V_exp is ignored by design.  The reference for phis_c is the
  soft-label phis_c field.
...
SCRIPT_VERSION = "ModelFin103-cycle-range-softlabel-only-v1"
DEFAULT_SOFT_LABEL_DIR = Path(r"C:\Users\Tiga_QJW\Desktop\ASSB_Scheme_V1\assb_soft_lable_cycle5-522_v1")
DEFAULT_OCP_DIR = Path(r"C:\Users\Tiga_QJW\Desktop\ASSB_Scheme_V1\ocp_estimation_outputs")
DEFAULT_OUTPUT_DIR = Path("EvalFin_103_cycles5_100_v1_softlabel_only")
...
def _filter_solution_by_cycle(sol: Dict[str, Any], cycle_from: int, cycle_to: int) -> Dict[str, Any]:
    """Return a shallow copy of solution arrays filtered by inclusive cycle range.

    The physical/global time coordinate is intentionally not shifted.  This keeps
    the evaluator consistent with models trained on continuous t_global_s.
    """
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate ModelFin_103 against a selected cycle range of continuous ASSB soft labels only.")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if np.any(cycle_id >= 0):
        # Potential branches use time-point cycle IDs; concentration branches use
        # repeated cycle IDs for each sampled radial point.  Concentration and
        # theta per-cycle metrics are therefore based on the sampled cs rows.
        ...
        for var, theta_key, csmax_key in (("cs_a", "theta_a", "csanmax"), ("cs_c", "theta_c", "cscamax")):
            csmax = metrics_global["model_params"].get(csmax_key, float("nan"))
            if np.isfinite(csmax) and csmax != 0:
                cycle_rows[theta_key] = _per_cycle_metrics(
                    results[var]["cycle_id"],
                    results[var]["label"] / csmax,
                    results[var]["pred"] / csmax,
                )
    ...
    np.savez_compressed(
        output_dir / "eval_sampled_arrays_cycles5_100_softlabel_only.npz",
