...
foreach ($File in $Required) {
    if (-not (Test-Path $File -PathType Leaf)) {
        throw "Required D18-S0/S1-FIX file is missing: $File"
    }
}
...
Write-Host "[D18-S0/S1-FIX] This run is diagnostic-only and will not start D18-S2 training."
& $PythonExe @Arguments
if ($LASTEXITCODE -ne 0) {
    throw "D18-S0/S1-FIX exited with code $LASTEXITCODE"
}

Write-Host "[D18-S0/S1-FIX] Completed. Default output:"
