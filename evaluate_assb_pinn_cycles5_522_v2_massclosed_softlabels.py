...
# -*- coding: utf-8 -*-
"""
Evaluate ModelFin_106 raw predictions against v2 mass-closed candidate soft labels over cycle5-522.

Default task
------------
Model:      ModelFin_106
Soft labels:C:\\Users\\Tiga_QJW\\Desktop\\ASSB_Scheme_V1\\assb_soft_labels_cycle5_522_v2_massclosed_candidate
Cycles:     5-522, inclusive
...
same model-loading and output-rescaling logic as the existing cycle-range
...
SCRIPT_VERSION = "ModelFin106-cycle5-522-v2-massclosed-raw-softlabel-only-v1"
...
DEFAULT_OUTPUT_DIR = Path("EvalFin_106_cycles5_522_v2_massclosed_candidate_linearGauge_raw_softlabel_only")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate ModelFin_106 raw outputs against v2 mass-closed candidate continuous ASSB soft labels, cycle5-522 by default."
    )
    parser.add_argument("--repo_root", type=Path, default=None, help="Project root. Default: script directory")
    parser.add_argument("--model_dir", type=Path, default=Path("ModelFin_106"), help="Model directory. Default: %(default)s")
    ...
    parser.add_argument("--cycle_to", type=int, default=522, help="Last cycle_id to evaluate, inclusive. Default: %(default)s")
    ...
    parser.add_argument("--max_cs_time_points", type=int, default=20000, help="Max sampled concentration time rows after cycle filtering; <=0 means all. Default: %(default)s")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    npz_name = f"eval_sampled_arrays_cycles{args.cycle_from}_{args.cycle_to}_v2_massclosed_softlabel_only.npz"
    np.savez_compressed(
        output_dir / npz_name,
