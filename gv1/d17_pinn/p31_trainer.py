# -*- coding: utf-8 -*-
"""D17-P3.1 12-profile mechanism smoke trainer.

P3.1 extends the P3 fix2 mechanism smoke from 6 profiles to 12 profiles and
adds a voltage-recovery / loss-scale audit stage.  It keeps the D17 boundary:
...
def train_p31_mechanism_smoke(cfg: Mapping[str, Any], out_dir: str | Path) -> Dict[str, Any]:
    ...
    if flagged_selected:
        raise RuntimeError(f"D17-P3.1 selected flagged_probe records, refusing training: {flagged_selected}")
    if missing_replay:
        raise RuntimeError(f"D17-P3.1 selected records without replay_npz, refusing training: {missing_replay}")
    ...
    model = D17MechanisticPINN(
        ...
        voltage_inverse_residual_amp_V=float(cfg_get(cfg, "model.voltage_inverse_residual_amp_V", 0.18)),
    ).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for i, batch in enumerate(batches):
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(f"Non-finite D17-P3.1 loss at epoch {epoch}, profile {i}: {loss}")
    ...
    with torch.no_grad():
        for i, (rec, batch) in enumerate(zip(selected_records, batches)):
            ...
            np.savez_compressed(
                out_dir / "predictions" / f"D17_P31_PROFILE_{i:02d}_PRED_OBS_ONLY.npz",
    ...
    (out_dir / "D17_P31_LOSS_SCALE_AUDIT.json").write_text(json.dumps(_jsonable(loss_audit), ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    summary = {
        "protocol": "D17-P3.1_12PROFILE_MECHANISM_SMOKE",
        "status": status,
        "reasons": reasons,
        "seed": seed,
        "device": str(device),
        ...
        "no_state_label_policy": {
            ...
            "softlabel_npz": "report-only path in manifest; not loaded by P3.1 trainer",
        ...
        "mechanism_notes": {
            ...
            "p31_additions": "12-profile balanced smoke + voltage-recovery phase + loss-scale audit",
            "voltage_not_copied": "V(t) is not assigned to V_pred; inverse residual is bounded/gated and reported separately",
        },
        "prior_snapshot": prior_to_jsonable(prior),
        "outputs": {
            "summary_json": str(out_dir / "D17_P31_12PROFILE_SMOKE_SUMMARY.json"),
            "loss_scale_audit_json": str(out_dir / "D17_P31_LOSS_SCALE_AUDIT.json"),
    ...
    (out_dir / "D17_P31_12PROFILE_SMOKE_SUMMARY.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    return summary
