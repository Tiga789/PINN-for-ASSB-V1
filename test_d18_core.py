...
from gv1.d18_cycleaware.common import dump_json, expand_template, write_csv
from gv1.d18_cycleaware.metrics import radial_deviation, regression_metrics, shell_volume_weights, volume_mean
from gv1.d18_cycleaware.model_scaffold import CycleAwareOperator, D18ModelConfig, ZeroMeanRadialBasis, synthetic_architecture_check
...
class ArchitectureTests(unittest.TestCase):
    def test_synthetic_architecture_contract(self) -> None:
        ...
        self.assertTrue(result["radial_shape_pointwise_bound_ok"])
        self.assertTrue(result["theta_bounds_ok"])
        self.assertTrue(result["concentration_bounds_ok"])
        self.assertEqual(result["theta_outside_fraction_a"], 0.0)
        self.assertEqual(result["theta_outside_fraction_c"], 0.0)
        self.assertLess(result["zero_volume_mean_max_abs_a"], 1e-5)
        self.assertLess(result["zero_volume_mean_max_abs_c"], 1e-5)

    def test_combined_radial_basis_is_pointwise_bounded(self) -> None:
        basis = ZeroMeanRadialBasis(17, 6)
        coeff = torch.tensor([[1.0e6, -2.0e6, 3.0e6, -4.0e6, 5.0e6, -6.0e6]])
        shape = basis(coeff)
        self.assertLessEqual(float(torch.max(torch.abs(shape))), 1.000001)
        self.assertLess(float(torch.max(torch.abs(basis.weighted_mean(shape)))), 1e-6)

    def test_extreme_cbar_is_clamped_to_theta_admissible_interval(self) -> None:
        cfg = D18ModelConfig(cycle_feature_dim=4, local_feature_dim=5, cycle_hidden_dim=8, local_hidden_dim=8, fused_hidden_dim=12)
        model = CycleAwareOperator(cfg).eval()
        b, c, t = 2, 3, 11
        with torch.no_grad():
            out = model(
                cycle_features=torch.randn(b, c, 4),
                local_features=torch.randn(b, t, 5),
                cycle_index=torch.zeros(b, t, dtype=torch.long),
                cbar=torch.tensor([[[-10.0, 10.0]]]).expand(b, t, 2).clone(),
                potential_baseline=torch.zeros(b, t, 2),
                branch_id=torch.tensor([0, 1]),
                theta_offset=torch.tensor([[-0.5, 1.5], [-0.5, 1.5]]),
                theta_scale=torch.tensor([[2.0, -2.0], [2.0, -2.0]]),
            )
        self.assertGreaterEqual(float(out["theta_a"].min()), -1e-6)
        self.assertLessEqual(float(out["theta_a"].max()), 1.0 + 1e-6)
        self.assertGreaterEqual(float(out["theta_c"].min()), -1e-6)
        self.assertLessEqual(float(out["theta_c"].max()), 1.0 + 1e-6)


class StrictOutputTests(unittest.TestCase):
    def test_json_replaces_nonfinite_with_null(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_text:
            path = Path(tmp_text) / "strict.json"
            dump_json({"nan": float("nan"), "posinf": float("inf"), "array": np.array([1.0, np.nan])}, path)
            text = path.read_text(encoding="utf-8")
            self.assertNotIn("NaN", text)
            self.assertNotIn("Infinity", text)
            loaded = json.loads(text)
            self.assertIsNone(loaded["nan"])
            self.assertIsNone(loaded["posinf"])
            self.assertIsNone(loaded["array"][1])

    def test_empty_csv_keeps_schema_header(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_text:
            path = Path(tmp_text) / "empty.csv"
            write_csv([], path, fieldnames=["case_id", "message"])
            self.assertEqual(path.read_text(encoding="utf-8-sig").strip(), "case_id,message")
